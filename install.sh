...
_install_arch_linux() {
  ...
  if command -v pacman &>/dev/null; then
    ...
    if [[ "${pacman_version}" != "${LATEST_VERSION#v}"* ]]; then
      ...
      sudo pacman -S --noconfirm cargo-cyndra
    fi
  else
...
_install_alpine_linux() {
  ...
  if command -v apk &>/dev/null; then
    if apk search -q cargo-cyndra; then
      echo "cargo-cyndra is not available in the testing repository. Do you want to enable the testing repository? (y/n)"
    ...
    if ! apk info cargo-cyndra; then
      echo "Installing cargo-cyndra"
      apk add cargo-cyndra@testing
    else
      ...
      if [[ "${apk_version}" != "${LATEST_VERSION#v}"* ]]; then
        ...
        echo "cargo-cyndra is already up to date."
      fi
    fi
  else
...
_install_binary() {
  ...
  *)
  ...
  curl -LO "$REPO_URL/releases/download/$LATEST_VERSION/cargo-cyndra-$LATEST_VERSION-$target.tar.gz"
  tar -xzf "cargo-cyndra-$LATEST_VERSION-$target.tar.gz"
  echo "Installing to $HOME/.cargo/bin/cargo-cyndra"
  mv "cargo-cyndra-$target-$LATEST_VERSION/cargo-cyndra" "$HOME/.cargo/bin/"
  echo "Installing to $HOME/.cargo/bin/cyndra"
  mv "cargo-cyndra-$target-$LATEST_VERSION/cyndra" "$HOME/.cargo/bin/"
  popd >/dev/null || exit 1
  if [[ ":$PATH:" != *":$HOME/.cargo/bin:"* ]]; then
    echo "Add $HOME/.cargo/bin to PATH to run cargo-cyndra"
  fi
}
...
_install_with_cargo() {
  ...
  cargo install --locked cargo-cyndra
}
...
_install_default() {
  ...
  if command -v cargo-binstall &>/dev/null; then
    ...
    cargo binstall -y --locked cargo-cyndra
    return 0
  fi
...
if command -v cargo-cyndra &>/dev/null; then
  if [[ "$(cargo-cyndra -V)" = *"${LATEST_VERSION#v}" ]]; then
    echo "cargo-cyndra is already at the latest version!"
    exit
  else
    echo "Updating cargo-cyndra to $LATEST_VERSION"
  fi
fi
