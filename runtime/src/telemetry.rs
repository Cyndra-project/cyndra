...
fn resource(crate_name: &'static str, package_version: &'static str) -> Resource {
    let project_name = std::env::var("cyndra_PROJECT_NAME").ok();
    ...
    TelemetryResourceDetector
        ...
        .merge(Box::new(Resource::from_schema_url(
            [
                ...
                Some(KeyValue::new("cyndra.project.crate.name", crate_name)),
                Some(KeyValue::new(
                    "cyndra.deployment.env",
                    std::env::var("cyndra_ENV")
                        .ok()
                        .unwrap_or("unknown".into()),
                )),
                std::env::var("cyndra_PROJECT_ID")
                    .ok()
                    .map(|value| KeyValue::new("cyndra.project.id", value)),
                project_name.map(|value| KeyValue::new("cyndra.project.name", value)),
            ]
            .into_iter()
            .flatten(),
            SCHEMA_URL,
...
pub fn otel_tracing_subscriber(
    ...
    let layers: OtelLayerType = MetricsLayer::new(meter.clone())
        .and_then(OpenTelemetryLayer::new(tracer.tracer("cyndra-telemetry")))
        .and_then(LogCourier::new(logger.logger("cyndra-telemetry")));
