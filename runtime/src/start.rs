...
fn parse_args() -> anyhow::Result<()> {
    if std::env::args().any(|arg| arg == "--port") {
        anyhow::bail!("Outdated argument detected (--port). Upgrade your Cyndra CLI.");
    }

    if std::env::var("cyndra_ENV").is_err() {
        anyhow::bail!("cyndra_ENV is required to be set on cyndra.dev");
    }
...
pub async fn start(
    ...
    if let Err(e) = parse_args() {
        ...
        let help_str = "[HINT]: Run your Cyndra app with `cyndra run`";
    ...
    tracing::warn!("Default tracing subscriber initialized (https://docs.cyndra.dev/docs/logs)");
