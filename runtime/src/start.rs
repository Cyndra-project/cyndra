...
#[derive(Default)]
struct Args {
    /// Enable compatibility with beta platform [env: cyndra_BETA]
    beta: bool,
...
impl Args {
    ...
    fn parse() -> anyhow::Result<Self> {
        ...
        args.beta = std::env::var("cyndra_BETA").is_ok();

        if args.beta {
            if std::env::var("cyndra_ENV").is_err() {
                return Err(anyhow::anyhow!("cyndra_ENV is required to be set on beta"));
            }
...
pub async fn start(loader: impl Loader + Send + 'static, runner: impl Runner + Send + 'static) {
    ...
    let args = match Args::parse() {
        ...
        Err(e) => {
            ...
            let help_str = "[HINT]: Run your Cyndra app with `cyndra run` or `cargo cyndra run`";
        ...
        registry()
            ...
            .with(
                ...
                EnvFilter::try_from_default_env()
                    ...
                    .or_else(|_| EnvFilter::try_new("info,cyndra=trace"))
                    .unwrap(),
            )
            .init();

        if args.beta {
            warn!("Default tracing subscriber initialized (https://docs.cyndra.dev/docs/logs)");
        } else {
            warn!("Default tracing subscriber initialized (https://docs.cyndra.rs/configuration/logs)");
        }
    }
