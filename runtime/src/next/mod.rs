...
use cyndra_common::wasm::{Bytesable, Log, RequestWrapper, ResponseWrapper};
...
impl Runtime for AxumWasm {
    async fn load(
        ...
        trace!(wasm_path, "loading cyndra-next project");
...
impl Router {
    ...
    async fn handle_request(
        ...
        self.linker
            .get(&mut store, "axum", "__cyndra_Axum_call")
...
pub mod tests {
    ...
    async fn axum() {
        ...
        let router = RouterBuilder::new()
            .unwrap()
            .src("tests/resources/axum-wasm-expanded/target/wasm32-wasi/debug/cyndra_axum_expanded.wasm")
            .build()
            .unwrap();
