...
use cyndra_common::{extract_propagation::ExtractPropagationLayer, secrets::Secret};
...
use cyndra_service::{ResourceFactory, Service};
use tokio::sync::{
...
pub async fn start(loader: impl Loader + Send + 'static, runner: impl Runner + Send + 'static) {
    // `--version` overrides any other arguments. Used by cargo-cyndra to check compatibility on local runs.
    ...
    let args = match Args::parse() {
        ...
        Err(e) => {
            ...
            let help_str = "[HINT]: Run your Cyndra app with `cargo cyndra run`";
    ...
    #[cfg(feature = "setup-tracing")]
    {
        ...
        tracing_subscriber::registry()
            ...
            .with(
                ...
                tracing_subscriber::EnvFilter::try_from_default_env()
                    ...
                    .or_else(|_| tracing_subscriber::EnvFilter::try_new("info,cyndra=trace"))
                    .unwrap(),
            )
            .init();
        ...
        println!(
            "{}",
            "Cyndra's default tracing subscriber is initialized!".yellow(),
        );
        println!("To disable it and use your own, check the docs: https://docs.cyndra.rs/configuration/logs");
    }
...
#[async_trait]
pub trait Loader {
    async fn load(self, factory: ResourceFactory) -> Result<Vec<Vec<u8>>, cyndra_service::Error>;
}
...
where
    ...
    O: Future<Output = Result<Vec<Vec<u8>>, cyndra_service::Error>> + Send,
{
    async fn load(self, factory: ResourceFactory) -> Result<Vec<Vec<u8>>, cyndra_service::Error> {
        (self)(factory).await
    }
...
pub trait Runner {
    ...
    async fn run(self, resources: Vec<Vec<u8>>) -> Result<Self::Service, cyndra_service::Error>;
}
...
where
    ...
    O: Future<Output = Result<S, cyndra_service::Error>> + Send,
    S: Service,
{
    type Service = S;
