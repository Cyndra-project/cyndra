...
```bash
cargo test axum --features cyndra-axum -- --nocapture
```

Load and run:

```bash
cargo run --features cyndra-axum -- --axum --provisioner-address http://localhost:8000
```
...
cargo build
# then
/home/<path to cyndra repo>/target/debug/cyndra-runtime --legacy --provisioner-address http://localhost:6001
```
