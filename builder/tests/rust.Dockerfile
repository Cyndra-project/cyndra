...
# Files and directories used by the Cyndra build process:
...
RUN touch /app/Cyndra.toml
RUN touch /app/cyndra_prebuild.sh
RUN touch /app/cyndra_postbuild.sh
RUN touch /app/cyndra_setup_container.sh
...
COPY cyndra_prebuild.sh .
RUN bash cyndra_prebuild.sh
...
RUN bash cyndra_postbuild.sh

RUN mv /app/target/release/hello /executable



RUN for path in \$(tq -r '.build.assets // .build_assets // [] | join(" ")' Cyndra.toml); do find "\$path" -type f -exec echo Copying \{\} \; -exec install -D \{\} /build_assets/\{\} \; ; done


FROM runtime-base AS runtime
WORKDIR /app

COPY --from=builder /app/cyndra_setup_container.sh /tmp
RUN bash /tmp/cyndra_setup_container.sh; rm /tmp/cyndra_setup_container.sh
