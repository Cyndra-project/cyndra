...
impl TempCargoHome {
    /// Initialize a new `TempCargoHome` with a `cyndra-service`
    /// patch unless `cyndra_CARGO_HOME` is set, then use that. With
...
",
        ...
        Command::new(CARGO.as_os_str())
            ...
            .current_dir(WORKSPACE_ROOT.as_path())
            .output()
            .ensure_success("failed to `cargo build --bin cargo-cyndra`");
    ...
    pub fn wait_deployer_ready(&self, project_path: &str, mut timeout: Duration) {
        ...
        while !timeout.is_zero() {
            ...
            if env::var("cyndra_API_KEY").is_err() {
                run.env("cyndra_API_KEY", "test-key");
            }
