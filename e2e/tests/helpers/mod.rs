...
impl Services {
        ...
        C: Into<Color>,
    {
        ...
        Command::new("docker")
            .args(["network", "create", "--driver", "bridge", "cyndra-net"])
            .spawn()
            .unwrap()
            .wait()
            .unwrap();
        ...
        let provisioner_container = format!("cyndra_provisioner_{}_{}", api.target, api.id);
        ...
        let api_container = format!("cyndra_api_{}_{}", api.target, api.id);
        let mut run = Command::new("docker");
        run.args([
            "run",
            "--name",
    ...
    fn build_image(service: &str, target: &str, id: &str) -> String {
        let image = format!("cyndra_{service}_{target}_{id}");
        ...
        I: IntoIterator<Item = &'s str>,
    {
        ...
        build
            .args(["build", "--bin", "cargo-cyndra"])
            .current_dir("../");
        spawn_and_log(&mut build, client_target.as_str(), Color::White)
            .wait()
            .ensure_success("failed to build `cargo-cyndra`");

        let mut run = Command::new("../../../target/debug/cargo-cyndra");
        run.args(args)
            .current_dir(path)
            .env("cyndra_API", format!("http://{}", self.api_addr));
