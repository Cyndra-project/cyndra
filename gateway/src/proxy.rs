...
use cyndra_common::backends::cache::{CacheManagement, CacheManager};
use cyndra_common::backends::headers::XCyndraProject;
use cyndra_common::constants::DEPLOYER_SERVICE_HTTP_PORT;
use cyndra_common::models::error::InvalidProjectName;
...
async fn proxy(
    ...
    let span = debug_span!("proxy", http.method = %req.method(), http.host = field::Empty, http.uri = %req.uri(), http.status_code = field::Empty, cyndra.project.name = field::Empty);
