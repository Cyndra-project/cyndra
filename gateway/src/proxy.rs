...
lazy_static::lazy_static! {
    pub static ref X_cyndra_PROJECT: HeaderName = HeaderName::from_static("x-cyndra-project");
}

pub struct XCyndraProject(ProjectName);
...
impl UserProxy {
    async fn proxy(self, mut req: Request<Body>) -> Result<Response, Error> {
        ...
        req.headers_mut()
            .typed_insert(XCyndraProject(project_name.clone()));
