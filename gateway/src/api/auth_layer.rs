...
use cyndra_common::backends::{auth::ConvertResponse, cache::CacheManagement};
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        if req.uri().path() == "/logout" {
            ...
            if let Ok(Some(cookie)) = req.headers().typed_try_get::<Cookie>() {
                if let Some(key) = cookie.get("cyndra.sid").map(|id| id.to_string()) {
        ...
        if forward_to_auth {
            ...
            Box::pin(async move {
                ...
                if let Some(cookie) = req.headers().typed_get::<Cookie>() {
                    if let Some(id) = cookie.get("cyndra.sid") {
