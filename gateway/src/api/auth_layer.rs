...
impl<S> Layer<S> for CyndraAuthLayer {
    type Service = CyndraAuthService<S>;
...
#[derive(Clone)]
pub struct CyndraAuthService<S> {
    inner: S,
    auth_uri: Uri,
    gateway_admin_key: String,
    cache_manager: Arc<Box<dyn CacheManagement<Value = String>>>,
...
impl<S> Service<Request<Body>> for CyndraAuthService<S>
where
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        if req.uri().path() == "/logout" {
            if let Ok(Some(cookie)) = req.headers().typed_try_get::<Cookie>() {
                if let Some(cache_key) = cookie.get("cyndra.sid").map(|id| id.to_string()) {
                    self.cache_manager.invalidate(&cache_key);
                }
            };
        }
...
fn make_token_request(
    ...
    if let Some(key) = gateway_admin_key {
        trace!("XCyndraAdminSecret header inserted for token request");
        token_request
            .headers_mut()
            .expect("manual request to be valid")
            .typed_insert(XCyndraAdminSecret(key.to_string()));
    }
