...
use cyndra_backends::client::permit::Team;
...
use cyndra_common::models::{service, team};
...
async fn override_create_service(scoped_user: ScopedUser) -> Result<Response<Body>, ApiError> {
    // Creating new deployments on the cyndra.rs platform is deprecated as of the first of January
    // 2025.
    return Err(Deprecated(
        "Creating new deployments on the cyndra.rs platform has been deprecated.".to_string(),
    )
    .into());
}
...
#[instrument(skip_all, fields(cyndra.team.name = %team_name, cyndra.team.id = field::Empty))]
async fn create_team(
    ...
    Span::current().record("cyndra.team.id", &team.id);
...
#[instrument(skip_all, fields(cyndra.team.id = %team_id))]
...
#[instrument(skip_all, fields(cyndra.team.id = %team_id, cyndra.project.id = %project_id))]
