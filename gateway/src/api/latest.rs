...
use cyndra_backends::client::permit::Organization;
...
use cyndra_common::claims::{Claim, Scope, EXP_MINUTES};
use cyndra_common::models::error::{ErrorKind, InvalidOrganizationName};
use cyndra_common::models::{admin::ProjectResponse, project, stats};
use cyndra_common::models::{organization, service};
    ...
    let version = project
        ...
        .and_then(|container_inspect_response| {
            container_inspect_response.image.and_then(|inner| {
                inner
                    .strip_prefix("public.ecr.aws/cyndra/deployer:v")
...
async fn override_create_service(
    ...
    tokio::spawn(async move {
        let event = async_posthog::Event::new("cyndra_api_start_deployment", &user_id);
...
#[instrument(skip_all, fields(cyndra.organization.name = %organization_name, cyndra.organization.id = field::Empty))]
async fn create_organization(
    ...
    Span::current().record("cyndra.organization.id", &org.id);

    Ok("Organization created".to_string())
}

#[instrument(skip_all, fields(cyndra.organization.id = %organization_id))]
...
#[instrument(skip_all, fields(cyndra.organization.id = %organization_id, cyndra.project.id = %project_id))]
