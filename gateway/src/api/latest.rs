...
async fn delete_project(
    ...
    if res.status() != StatusCode::NOT_FOUND {
        ...
        let summary: cyndra_common::models::service::Summary = serde_json::from_slice(&body_bytes)
            .map_err(|e| Error::source(ErrorKind::Internal, e))?;
    ...
    if res.status() != StatusCode::NOT_FOUND {
        ...
        let body_bytes = hyper::body::to_bytes(res.into_body())
            .await
            .map_err(|e| Error::source(ErrorKind::Internal, e))?;
        let resources: Vec<cyndra_common::resource::Response> =
        ...
        let resources = resources
            ...
            .filter(|resource| {
                matches!(
                    resource.r#type,
                    cyndra_common::resource::Type::Database(_)
                        | cyndra_common::resource::Type::Secrets
                )
            })
