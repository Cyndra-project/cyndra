...
use cyndra_service::{error::CustomError, Factory};
...
impl Factory for LocalFactory {
    async fn get_sql_connection_string(&mut self) -> Result<String, cyndra_service::Error> {
        ...
        let container_name = format!("cyndra_{}_postgres", self.project);
...
impl LocalFactory {
    async fn wait_for_ready(&self, container_name: &str) -> Result<(), cyndra_service::Error> {
        loop {
