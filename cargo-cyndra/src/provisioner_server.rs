    ...
    async fn get_container(
        ...
        match self.docker.inspect_container(container_name, None).await {
            ...
            Err(error) => {
                ...
                error!(
                    "Make sure Docker is installed and running. For more help: https://docs.cyndra.dev/docs/local-run#docker-engines"
                );
    ...
    async fn provision(
        ...
        Ok(match (method, uri) {
            ...
            (Method::POST, "/projects/proj_LOCAL/resources") => {
                ...
                let cyndra_resource: ProvisionResourceRequestBeta =
                ...
                let response = match cyndra_resource.r#type {
                    ...
                    | ResourceTypeBeta::DatabaseAwsRdsPostgres => {
                        let config: DbInput =
                            serde_json::from_value(cyndra_resource.config.clone())
                        ...
                        let res = DatabaseResource::Info(
                            prov.provision_database(Request::new(DatabaseRequest {
                                ...
                                db_type: Some(
                                    database::Type::try_from(cyndra_resource.r#type)
                        ...
                        ResourceResponseBeta {
                            ...
                            config: cyndra_resource.config,
                            output: serde_json::to_value(res).unwrap(),
                        }
                    }
                    ResourceTypeBeta::Container => {
                        let config = serde_json::from_value(cyndra_resource.config.clone())
                            .context("deserializing resource config")?;
