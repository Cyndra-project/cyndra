//! Suggestions to be shown to users encountering errors while using cargo-cyndra.
...
pub fn reset_api_key_failed(err: anyhow::Error) -> anyhow::Error {
    ...
    println!();
    println!("If trying to log out and reset the API key at the same time fails repeteadly, please check Cyndra status at https://status.cyndra.rs or open a help thread on the Discord server.");
    err
}
