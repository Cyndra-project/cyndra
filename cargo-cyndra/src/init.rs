...
const cyndra_EXAMPLES_README: &str = "https://github.com/cyndra\
		     -hq/cyndra-examples#how-to-clone-run-and-deploy-an-example";
...
pub fn generate_project(dest: PathBuf, name: &str, temp_loc: TemplateLocation) -> Result<()> {
    ...
    edit_cyndra_toml(&path).context("Failed to edit Cyndra.toml")?;
...
fn setup_template(auto_path: &str) -> Result<TempDir> {
    ...
    if let Some(caps) = git_re.captures(auto_path) {
        ...
        if url.scheme() == "http" || url.scheme() == "https" {
		 ...
		 a template here: {cyndra_EXAMPLES_README}"
            );
...
fn edit_cyndra_toml(path: &Path) -> Result<()> {
    let path = path.join("Cyndra.toml");
    if !path.exists() {
        // Do nothing if template has no Cyndra.toml
        return Ok(());
    }
    let toml_str = read_to_string(&path)?;
    let mut doc = toml_str.parse::<Document>()?;

    // The Cyndra.toml project name override will likely already be in use,
...
mod tests {
    ...
    fn gix_clone_works() {
        let temp_dir = Builder::new()
            ...
            .unwrap();
        gix_clone(
            "https://github.com/cyndra-hq/awesome-cyndra.git",
            temp_dir.path(),
        )
        .unwrap();
