...
pub fn get_framework(init_args: &InitArgs) -> Box<dyn CyndraInit> {
    if init_args.actix_web {
        return Box::new(CyndraInitActixWeb);
    }
    if init_args.axum {
        return Box::new(CyndraInitAxum);
    }

    if init_args.rocket {
        return Box::new(CyndraInitRocket);
    }

    if init_args.tide {
        return Box::new(CyndraInitTide);
    }
    ...
    if init_args.salvo {
        return Box::new(CyndraInitSalvo);
    }

    if init_args.serenity {
        return Box::new(CyndraInitSerenity);
    }

    if init_args.warp {
        return Box::new(CyndraInitWarp);
    }

    if init_args.thruster {
        return Box::new(CyndraInitThruster);
    }
...
pub fn cargo_init(path: PathBuf) -> Result<()> {
    ...
    cargo_config
        .shell()
        .status("Created", format!("{} (cyndra) package", init_result))?;

    Ok(())
}

/// Performs cyndra init on the existing files generated by `cargo init --libs [path]`.
pub fn cargo_cyndra_init(path: PathBuf, framework: Box<dyn CyndraInit>) -> Result<()> {
...
mod cyndra_init_tests {
    ...
    fn test_get_framework_via_get_boilerplate_code() {
        ...
        let framework_inits: Vec<Box<dyn CyndraInit>> = vec![
            Box::new(CyndraInitActixWeb),
            Box::new(CyndraInitAxum),
            Box::new(CyndraInitRocket),
            Box::new(CyndraInitTide),
            Box::new(CyndraInitTower),
            Box::new(CyndraInitPoem),
            Box::new(CyndraInitSalvo),
            Box::new(CyndraInitSerenity),
            Box::new(CyndraInitWarp),
            Box::new(CyndraInitThruster),
        ];
    ...
    fn test_set_inline_table_dependency_features() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { features = ["test-feature"] }
        "#};
    ...
    fn test_set_cargo_dependencies_actix_web() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-actix-web"] }
            actix-web = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_axum() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-axum"] }
            axum = "1.0"
            sync_wrapper = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_rocket() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-rocket"] }
            rocket = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_tide() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-tide"] }
            tide = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_salvo() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-salvo"] }
            salvo = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_serenity() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["bot-serenity"] }
            anyhow = "1.0"
    ...
    fn test_set_cargo_dependencies_warp() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-warp"] }
            warp = "1.0"
        "#};
    ...
    fn test_set_cargo_dependencies_thruster() {
        ...
        let expected = indoc! {r#"
            [dependencies]
            cyndra-service = { version = "1.0", features = ["web-thruster"] }
