...
use regex::Regex;
use cyndra_common::constants::cyndra_EXAMPLES_README;
...
pub fn generate_project(
    ...
    // if the crate name was not updated, set it in Cyndra.toml instead
    edit_cyndra_toml(&path, (!crate_name_set).then_some(name))
        .context("Failed to edit Cyndra.toml")?;
...
fn setup_template(auto_path: &str) -> Result<TempDir> {
    ...
    if let Some(caps) = git_re.captures(auto_path) {
        ...
        if url.scheme() == "http" || url.scheme() == "https" {
            ...
            println!(
                "HINT: You can find examples of how to select a template here: {cyndra_EXAMPLES_README}"
            );
...
/// Remove or set the "name" field in Cyndra.toml based on what is needed.
fn edit_cyndra_toml(path: &Path, set_name: Option<&str>) -> Result<()> {
    let path = path.join("Cyndra.toml");

    if set_name.is_none() && !path.exists() {
        // Do nothing if template has no Cyndra.toml and the name should not be set
        return Ok(());
    }
...
fn create_ignore_file(path: &Path, name: &str) -> Result<()> {
    ...
    for rule in ["/target", ".cyndra-storage", "Secrets*.toml"] {
...
mod tests {
    ...
    fn gix_clone_works() {
        let temp_dir = Builder::new()
            .prefix("cyndra-clone-test")
            .tempdir()
            .unwrap();
        gix_clone(
            "https://github.com/cyndra-hq/cyndra-examples.git",
            temp_dir.path(),
        )
        .unwrap();
