...
use cyndra_proto::runtime::{self, LoadRequest, StartRequest, SubscribeLogsRequest};
...
use cyndra_service::builder::{build_workspace, Runtime};
use std::fmt::Write;
...
{}"#,
    ...
    fn make_archive(&self) -> Result<Vec<u8>> {
        ...
        if secrets_path.exists() {
            tar.append_path_with_name(secrets_path, Path::new("cyndra").join("Secrets.toml"))?;
        }
...
fn check_version(runtime_path: &Path) -> Result<()> {
    ...
    if runtime_version == valid_version {
        ...
        Err(anyhow!(
            "cyndra-runtime and cargo-cyndra are not the same version"
        ))
    }
}
...
mod tests {
    ...
    fn find_root_directory_returns_proper_directory() {
        ...
        let root_dir = Cyndra::find_root_directory(&working_directory).unwrap();
