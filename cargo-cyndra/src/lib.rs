...
use indoc::printdoc;
use cyndra_common::claims::{ClaimService, InjectPropagation};
...
pub use args::{Command, DeployArgs, InitArgs, LoginArgs, ProjectArgs, RunArgs, CyndraArgs};
...
use cyndra_common::models::{project, secret};
...
impl Cyndra {
    ...
    async fn local_run(&self, run_args: RunArgs) -> Result<()> {
        let services = Cyndra::pre_local_run(self, &run_args).await?;
    ...
    async fn local_run(&self, run_args: RunArgs) -> Result<()> {
        ...
        for (i, service) in services.iter().enumerate() {
            Cyndra::add_runtime_info(
                Cyndra::spin_local_runtime(
                    &run_args,
                    service,
                    &provisioner_server,
                    i as u16,
            ...
            if let Some(Ok(msg)) = message {
                if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                    ...
                    match log_item.state.clone() {
                        ...
                        cyndra_common::deployment::State::Running
