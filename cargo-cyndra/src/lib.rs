...
use cyndra_common::{
    constants::{
        ...
        RESOURCE_SCHEMA_VERSION, cyndra_GH_ISSUE_URL, cyndra_IDLE_DOCS_URL,
...
impl Cyndra {
    ...
    async fn login(&mut self, login_args: LoginArgs) -> Result<CommandOutcome> {
        let api_key_str = match login_args.api_key {
            ...
            None => {
                ...
                println!("If your browser did not automatically open, go to {cyndra_LOGIN_URL}");
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<CommandOutcome> {
        ...
        println!("Successfully logged out of cyndra.");
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<CommandOutcome> {
        ...
        if self.beta {
            ...
            let package = packages
                .first()
                .expect("at least one cyndra crate in the workspace");
