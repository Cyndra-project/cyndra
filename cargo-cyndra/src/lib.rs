...
use cyndra_common::DeploymentStateMeta;
...
impl Cyndra {
    ...
    async fn init(&self, args: InitArgs) -> Result<()> {
        // Interface with cargo to initialize new lib package for cyndra
        ...
        // Fetch the latest cyndra-service version from crates.io
        ...
        let latest_cyndra_service =
            get_latest_dependency("cyndra-service", false, &manifest_path, Some(&url))
                .expect("Could not query the latest version of cyndra-service");
        let cyndra_version = latest_cyndra_service
            .version()
            .expect("No latest cyndra-service version available");

        // Insert cyndra-service to `[dependencies]` table
        let mut dep_table = Table::new();
        dep_table["cyndra-service"]["version"] = value(cyndra_version);
    ...
    async fn check_lib_version(&self, project_args: ProjectArgs) -> Result<()> {
        ...
        let current_cyndra_version = &cargo_doc["dependencies"]["cyndra-service"]["version"];
        let service_semver = match Version::parse(current_cyndra_version.as_str().unwrap()) {
            Ok(version) => version,
            Err(error) => return Err(anyhow!("Your cyndra-service version ({}) is invalid and should follow the MAJOR.MINOR.PATCH semantic versioning format. Error given: {:?}", current_cyndra_version.as_str().unwrap(), error.to_string())),
        };

        let server_version = client::cyndra_version(self.ctx.api_url()).await?;
