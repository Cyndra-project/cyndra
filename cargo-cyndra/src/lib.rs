...
use cyndra_common::models::resource::get_certificates_table_beta;
use cyndra_common::{
    constants::{
        headers::X_CARGO_cyndra_VERSION, API_URL_DEFAULT, DEFAULT_IDLE_MINUTES, EXAMPLES_REPO,
        EXECUTABLE_DIRNAME, RESOURCE_SCHEMA_VERSION, RUNTIME_NAME, cyndra_GH_ISSUE_URL,
...
impl Cyndra {
    ...
    pub async fn run(
        ...
        if self.beta {
            ...
            eprintln!("This command is not supported on the legacy platform. Set --beta or cyndra_BETA=true.");
            return Ok(CommandOutcome::Ok);
        }
