...
impl Cyndra {
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        ...
        println!(" -> Use `cyndra login` to log in again.");
    ...
    async fn local_run_beta(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let secrets = Cyndra::get_secrets_beta(&run_args.secret_args, working_directory)?
            .unwrap_or_default();
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        let secrets = Cyndra::get_secrets_beta(&args.secret_args, working_directory)?;
