...
use cyndra_common::{
    constants::{
        ...
        cyndra_IDLE_DOCS_URL, cyndra_LOGIN_URL, cyndra_LOGIN_URL_BETA, STORAGE_DIRNAME,
...
use crate::util::{
    ...
    is_dirty, open_gh_issue, update_cargo_cyndra,
};
...
impl Cyndra {
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        if logout_args.reset_api_key {
            ...
            println!(" -> Go to {cyndra_LOGIN_URL} to get a new one.\n");
        }
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        loop {
            if let Some(Ok(msg)) = stream.next().await {
                if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                    let log_item = match serde_json::from_str::<cyndra_common::LogItem>(&line) {
