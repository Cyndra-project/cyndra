...
use cyndra_common::{
    constants::{
        ...
        cyndra_CLI_DOCS_URL, cyndra_GH_ISSUE_URL, cyndra_IDLE_DOCS_URL,
        cyndra_INSTALL_DOCS_URL, cyndra_LOGIN_URL, STORAGE_DIRNAME,
    },
...
impl Cyndra {
    ...
    pub async fn parse_args_and_run(self) -> Result<CommandOutcome> {
        ...
        let args = CyndraArgs::from_arg_matches(&matches)
    ...
    async fn init(
        ...
        let template = match git_template {
            ...
            None => {
                println!(
                    "Cyndra works with a range of web frameworks. Which one do you want to use?"
                );
        ...
        printdoc!(
            ...
            Hint: Check the examples repo for a full list of templates:
                  {EXAMPLES_REPO}
            Hint: You can also use `cargo cyndra init --from` to clone templates.
                  See {cyndra_CLI_DOCS_URL}
                  or run `cargo cyndra init --help`
            "
        );
        println!();
    ...
    async fn local_run(&self, mut run_args: RunArgs) -> Result<CommandOutcome> {
        ...
        for (i, service) in services.iter().enumerate() {
            signal_received = tokio::select! {
                res = Cyndra::spin_local_runtime(&run_args, service, &provisioner_server, i as u16, provisioner_port) => {
                    Cyndra::add_runtime_info(res.unwrap(), &mut runtimes, &[&provisioner_server]).await?;
                    false
                },
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<CommandOutcome> {
        ...
        if deployment_req.data.len() > CREATE_SERVICE_BODY_LIMIT {
            bail!(
                ...
                Run with `RUST_LOG="cargo_cyndra=debug"` to see which files are being packed."#,
...
mod tests {
    ...
    fn get_archive_entries(project_args: ProjectArgs) -> Vec<String> {
        ...
        let archive = cyndra.make_archive().unwrap();
