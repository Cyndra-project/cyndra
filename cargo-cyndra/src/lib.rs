...
use cyndra_common::{
    constants::{
        headers::X_CARGO_cyndra_VERSION, API_URL_DEFAULT, API_URL_DEFAULT_BETA,
        DEFAULT_IDLE_MINUTES, EXAMPLES_REPO, EXECUTABLE_DIRNAME, RESOURCE_SCHEMA_VERSION,
        RUNTIME_NAME, cyndra_IDLE_DOCS_URL, cyndra_LEGACY_NEW_PROJECT, STORAGE_DIRNAME,
    ...
    resource::{self, ResourceInput, CyndraResourceOutput},
...
use cyndra_service::{
    builder::{async_cargo_metadata, build_workspace, find_cyndra_packages, BuiltService},
    runner, Environment,
};
...
pub use crate::args::{Command, ProjectArgs, RunArgs, CyndraArgs};
...
use crate::util::{
    ...
    is_dirty, open_gh_issue, read_ws_until_text, update_cargo_cyndra,
};

const VERSION: &str = env!("CARGO_PKG_VERSION");

/// Returns the args and whether the PATH arg of the init command was explicitly given
pub fn parse_args() -> (CyndraArgs, bool) {
    let matches = CyndraArgs::command().get_matches();
    let args =
        CyndraArgs::from_arg_matches(&matches).expect("args to already be parsed successfully");
    ...
    registry()
        ...
        .with(
            ...
            EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                if debug {
                    EnvFilter::new("info,cargo_cyndra=trace,cyndra=trace")
                } else {
                    EnvFilter::default()
                }
            }),
...
impl Binary {
    pub fn name(&self) -> String {
        match self {
            Self::CargoCyndra => "cargo-cyndra".to_owned(),
            Self::Cyndra => "cyndra".to_owned(),
        }
    }
}
...
impl Cyndra {
    ...
    pub async fn run(mut self, mut args: CyndraArgs, provided_path_to_init: bool) -> Result<()> {
        if self.bin == Binary::Cyndra {
            // beta is always enabled in `cyndra`
            args.beta = true;
        }
        self.beta = args.beta;
        if self.beta {
            ...
            if matches!(args.cmd, Command::Resource(ResourceCommand::Dump { .. })) {
                bail!("This command is not yet supported on the NEW platform (cyndra.dev).");
            }
        } else if matches!(
            args.cmd,
        ...
        let res = match args.cmd {
            ...
            Command::Upgrade { preview } => update_cargo_cyndra(preview).await,
        };
    ...
    async fn check_api_versions(&mut self) -> Result<()> {
        ...
        if let Ok(versions) = client.get_api_versions().await {
            ...
            // check cargo-cyndra version
            // should always be a valid semver
            let my_version = &semver::Version::from_str(VERSION).unwrap();
            let latest_version = &self.version_info.as_ref().unwrap().cargo_cyndra;
            if my_version != latest_version {
                ...
                let string = if semvers_are_compatible(my_version, latest_version) {
                    newer_version_exists.then(|| {
                        format!("Info: A newer version of cargo-cyndra exists ({latest_version}).")
                    })
                    ...
                    newer_version_exists.then(||
                        formatdoc! {"
                            Warning:
                                A newer version of cargo-cyndra exists ({latest_version}).
                                It is recommended to upgrade.
                                Refer to the upgrading docs: https://docs.cyndra.rs/configuration/cyndra-versions#upgrading-cyndra-version"
                        }
                    ).or_else(||
                        Some(formatdoc! {"
                            Warning:
                                Your version of cargo-cyndra ({my_version}) is newer than what the API expects ({latest_version}).
    ...
    /// Log in, initialize a project and potentially create the Cyndra environment for it.
    ///
    ...
    async fn init(
        ...
        if needs_login {
            println!("First, let's log in to your Cyndra account.");
            self.login(args.login_args.clone(), offline).await?;
            println!();
        } else if args.login_args.api_key.is_some() {
            self.login(args.login_args.clone(), offline).await?;
        } else if args.create_env {
            bail!("Tried to login to create a Cyndra environment, but no API key was set.")
        }
        ...
        if needs_name && !self.beta {
            printdoc! {"
                ...
                It will be hosted at ${{project_name}}.cyndraapp.rs, so choose something unique!
                "
            };
        }
        ...
        let template = match git_template {
            ...
            None => {
                ...
                let schema = if offline {
                    ...
                    get_templates_schema()
                        ...
                        .and_then(|s| {
                            ...
                            println!(
                                "{}",
                                "Template list with incompatible version found. Consider updating cargo-cyndra. Falling back to internal list."
                                    .yellow()
                            );
                ...
                if let Some(schema) = schema {
                    ...
                    println!("Cyndra works with many frameworks. Which one do you want to use?");
        ...
        let should_create_environment = if !interactive {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(if self.beta {
                    format!(r#"Create a project on Cyndra with the name "{name}"?"#)
                } else {
                    format!(
                        r#"Claim the project name "{name}" by starting a project container on Cyndra?"#
                    )
                })
                .default(true)
                .interact()?;
            if !should_create && !self.beta {
                println!(
                    ...
                    you start the project with `cargo cyndra project start`."
                )
            }
            println!();
            should_create
            ...
            if self.beta {
                ...
                if serenity_idle_hint {
                    printdoc!(
                        ...
                        project so that they don't go offline: {cyndra_IDLE_DOCS_URL}
                        "
                    );
                }
            }
        ...
        let api_key = match login_args.api_key {
            ...
            None => {
                if login_args.prompt || !self.beta {
                    // manual input requested (always the case on cyndra.rs)

                    if !login_args.prompt && !self.beta {
                        // if !beta, open console
                        let url = cyndra_LEGACY_NEW_PROJECT;
                    ...
                    // device auth flow via Cyndra Console
    ...
    async fn device_auth(&self, console_url: String) -> Result<String> {
        ...
        let _ = webbrowser::open(url);
        println!("Complete login in Cyndra Console to authenticate CLI.");
        println!("If your browser did not automatically open, go to {url}");
        println!();
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        if logout_args.reset_api_key {
            ...
            if self.beta {
                println!(" -> Use `cyndra login` to get a new one.")
            } else {
                println!(" -> Go to {cyndra_LEGACY_NEW_PROJECT} to get a new one.");
            }
            println!();
        }
    ...
    async fn stop(&self) -> Result<()> {
        ...
        wait_with_spinner(500, |i, pb| async move {
            ...
            let cleanup = move || {
                ...
                println!("{service_str}");
                println!("Run `cargo cyndra deploy` to re-deploy your service.");
            };
            ...
            if deployment.state == cyndra_common::deployment::State::Stopped {
                Ok(Some(cleanup))
            } else {
                Ok(None)
            }
    ...
    async fn resource_delete(
        ...
        println!(
            ...
            formatdoc! {"
                Note:
                    Remember to remove the resource annotation from your #[cyndra_runtime::main] function.
    ...
    async fn spin_local_runtime(
        ...
        let secrets = Cyndra::get_secrets(run_args, service)?;
        ...
        tokio::spawn(async move {
            while let Some(line) = reader.next_line().await.unwrap() {
                let log_item = LogItem::new(
                    deployment_id,
                    cyndra_common::log::Backend::Runtime(service_name_clone.clone()),
                    line,
                );
        ...
        let (resources, mocked_responses) =
            Cyndra::local_provision_phase(service_name.as_str(), resources, secrets).await?;
    ...
    async fn local_provision_phase(
        ...
        for (bytes, cyndra_resource) in
            resources
                ...
                .map(|(bytes, cyndra_resource)| {
                    if cyndra_resource.version == RESOURCE_SCHEMA_VERSION {
                        Ok((bytes, cyndra_resource))
                    } else {
                        Err(anyhow!("
                            Cyndra resource request for {} with incompatible version found. Expected {}, found {}. \
                            Make sure that this deployer and the Cyndra resource are up to date.
                            ",
                            cyndra_resource.r#type,
                            RESOURCE_SCHEMA_VERSION,
                            cyndra_resource.version
                        ))
                    }
                }).collect::<anyhow::Result<Vec<_>>>()?.into_iter()
        {
            match cyndra_resource.r#type {
                resource::Type::Database(db_type) => {
                    let config: DbInput = serde_json::from_value(cyndra_resource.config)
                ...
                resource::Type::Container => {
                    let config = serde_json::from_value(cyndra_resource.config)
                        .context("deserializing resource config")?;
        ...
        match runtime {
            ...
            None => {
                ...
                for rt_info in existing_runtimes {
                    ...
                    Cyndra::stop_runtime(&mut rt_info.0, &mut rt_info.1)
                        .await
                        .unwrap_or_else(|_| {
                            errored_out = true;
                        });
    ...
    async fn local_run(&self, mut run_args: RunArgs) -> Result<()> {
        ...
        for (i, service) in services.iter().enumerate() {
            ...
            signal_received = tokio::select! {
                res = Cyndra::spin_local_runtime(&run_args, service, i as u16) => {
                    match res {
                        Ok(runtime) => {
                            Cyndra::add_runtime_info(runtime, &mut runtimes).await?;
                        },
                ...
                _ = sigterm_notif.recv() => {
                    println!(
                        "cargo-cyndra received SIGTERM. Killing all the runtimes..."
                    );
                    true
                },
                _ = sigint_notif.recv() => {
                    println!(
                        "cargo-cyndra received SIGINT. Killing all the runtimes..."
                    );
                    true
                }
            };
        ...
        if signal_received {
            for (mut rt, mut rt_client) in runtimes {
                Cyndra::stop_runtime(&mut rt, &mut rt_client)
                    .await
                    .unwrap_or_else(|err| {
        ...
        for (mut rt, mut rt_client) in runtimes {
            ...
            signal_received = tokio::select! {
                ...
                _ = sigterm_notif.recv() => {
                    ...
                    Cyndra::stop_runtime(&mut rt, &mut rt_client).await.unwrap_or_else(|err| {
                        trace!(status = ?err, "stopping the runtime errored out");
                    });
                    true
                },
    ...
    async fn local_run_beta(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let service = services
            .first()
            .expect("at least one cyndra service")
            .to_owned();

        trace!(path = ?service.executable_path, "runtime executable");

        let secrets = Cyndra::get_secrets(&run_args, &service)?;
        Cyndra::find_available_port_beta(&mut run_args);
        if let Some(warning) = check_and_warn_runtime_version(&service.executable_path).await? {
            eprint!("{}", warning);
        }
        ...
        let exit_result = {
            ...
            tokio::select! {
                ...
                _ = sigterm_notif.recv() => {
                    eprintln!("cargo-cyndra received SIGTERM. Killing the runtime...");
                    None
                },
                _ = sigint_notif.recv() => {
                    eprintln!("cargo-cyndra received SIGINT. Killing the runtime...");
                    None
                }
            }
        };
        ...
        let exit_result = {
            ...
            tokio::select! {
                ...
                _ = ctrl_break_notif.recv() => {
                    eprintln!("cargo-cyndra received ctrl-break.");
                    None
                },
                _ = ctrl_c_notif.recv() => {
                    eprintln!("cargo-cyndra received ctrl-c.");
                    None
                },
                _ = ctrl_close_notif.recv() => {
                    eprintln!("cargo-cyndra received ctrl-close.");
                    None
                },
                _ = ctrl_logoff_notif.recv() => {
                    eprintln!("cargo-cyndra received ctrl-logoff.");
                    None
                },
                _ = ctrl_shutdown_notif.recv() => {
                    eprintln!("cargo-cyndra received ctrl-shutdown.");
                    None
                }
            }
        };
    ...
    async fn handle_signals() -> bool {
        ...
        tokio::select! {
            _ = ctrl_break_notif.recv() => {
                println!("cargo-cyndra received ctrl-break.");
                true
            },
            _ = ctrl_c_notif.recv() => {
                println!("cargo-cyndra received ctrl-c.");
                true
            },
            _ = ctrl_close_notif.recv() => {
                println!("cargo-cyndra received ctrl-close.");
                true
            },
            _ = ctrl_logoff_notif.recv() => {
                println!("cargo-cyndra received ctrl-logoff.");
                true
            },
            _ = ctrl_shutdown_notif.recv() => {
                println!("cargo-cyndra received ctrl-shutdown.");
                true
            }
            else => {
                false
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        let secrets = if self.beta {
            Cyndra::get_secrets_beta(&args.secret_args, working_directory)?
        } else {
            None
        };
        ...
        if self.beta {
            ...
            let packages = find_cyndra_packages(&metadata)?;
            // TODO: support overriding this
            let package = packages
                .first()
                .expect("Expected at least one crate with cyndra-runtime in the workspace");
            let package_name = package.name.to_owned();
            rust_build_args.package_name = Some(package_name);

            // activate cyndra feature if present
            let (no_default_features, features) = if package.features.contains_key("cyndra") {
                (true, Some(vec!["cyndra".to_owned()]))
            } else {
                (false, None)
            };
            ...
            rust_build_args.cyndra_runtime_version = package
                .dependencies
                .iter()
                .find(|dependency| dependency.name == RUNTIME_NAME)
                .expect("cyndra package to have runtime dependency")
                .req
                .comparators
                .first()
                // is "^0.X.0" when `cyndra-runtime = "0.X.0"` is in Cargo.toml
            ...
            // TODO: have all of the above be configurable in CLI and Cyndra.toml
        }
        ...
        if !self.beta && archive.len() > CREATE_SERVICE_BODY_LIMIT {
            bail!(
                ...
                Run with `cargo cyndra --debug` to see which files are being packed."#,
        ...
        while let Ok(Some(s)) = read_ws_until_text(&mut stream).await {
                ...
                && log_item.line.contains("Deployer version: ")
            {
                ...
                if latest_version > my_version {
                    self.version_warnings.push(
                                formatdoc! {"
                                    Warning:
                                        A newer version of cyndra-deployer is available ({latest_version}).
                                        Use `cargo cyndra project restart` to upgrade."
                                }
                                .yellow()
                                .to_string(),
                            )
            ...
            if !runtime_version_checked
                ...
                && log_item
                    .line
                    .contains("cyndra-runtime executable started (version ")
            {
                runtime_version_checked = true;
                let my_version = &log_item
                    .line
                    .split_once("cyndra-runtime executable started (version ")
                    .unwrap()
                    .1
                    .split_once(')')
                    .unwrap()
                ...
                if latest_version > my_version {
                    self.version_warnings.push(
                                formatdoc! {"
                                    Warning:
                                        A newer version of cyndra-runtime is available ({latest_version}).
                                        Update it and any other cyndra dependencies in Cargo.toml."
                                }
                                .yellow()
                                .to_string(),
                            )
                ...
                .any(|m| log_item.line.contains(m))
            {
                ...
                println!();
                println!("cargo cyndra logs {}", &deployment.id);
        ...
        if deployment.state != cyndra_common::deployment::State::Running {
            ...
            match deployment.state {
                ...
                cyndra_common::deployment::State::Crashed => {
                    println!(
                        "{}",
    ...
    async fn project_start(&self, idle_minutes: u64) -> Result<()> {
        ...
        if idle_minutes > 0 && !self.beta {
            ...
            println!("To change the idle time refer to the docs: {cyndra_IDLE_DOCS_URL}");
            println!();
        }
    ...
    async fn project_stop(&self) -> Result<()> {
        ...
        println!("Run `cargo cyndra project start` to recreate project environment on Cyndra.");
            ...
            println!(
                ...
                formatdoc!(
                    ...
                    WARNING:
                        ...
                        This will...
                        - Delete any databases, secrets, and cyndra-persist data in this project.
                        - Delete any custom domains linked to this project.
...
mod tests {
    ...
    async fn get_archive_entries(
        ...
        let mut cyndra = Cyndra::new(crate::Binary::CargoCyndra).unwrap();
        cyndra
    ...
    async fn make_archive_respect_rules() {
        let working_directory = canonicalize(path_from_workspace_root(
            "cargo-cyndra/tests/resources/archiving",
        ))
        .unwrap();
        ...
        let expected = vec![
            ...
            "Cyndra.toml",
            "asset1", // normal file
            "asset2", // .gitignore'd, but included in Cyndra.toml
            // asset3 is .ignore'd
            "asset4",                // .gitignore'd, but un-ignored in .ignore
            "asset5",                // .ignore'd, but included in Cyndra.toml
            "dist/dist1",            // .gitignore'd, but included in Cyndra.toml
