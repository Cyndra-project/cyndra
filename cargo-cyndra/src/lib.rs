...
impl Cyndra {
    ...
    async fn init(
        ...
        let should_create_environment = if self.beta {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(format!(
                    r#"Claim the project name "{}" by starting a project container on Cyndra?"#,
                    project_args
                        .name
                        .as_ref()
                        .expect("to have a project name provided")
    ...
    fn feedback(&self) -> Result<()> {
        ...
        println!("If your browser did not open automatically, go to {cyndra_GH_ISSUE_URL}");
    ...
    async fn spin_local_runtime(
        ...
        Cyndra::check_and_warn_runtime_version(&service.executable_path).await?;
...
async fn update_cargo_cyndra(preview: bool) -> Result<()> {
    if preview {
    ...
    let _ = tokio::process::Command::new("bash")
        .args(["-c", "curl -sSfL https://www.cyndra.rs/install | bash"])
        .kill_on_drop(true)
        .spawn()
    ...
    let _ = tokio::process::Command::new("powershell")
        .args(["-Command", "iwr https://www.cyndra.rs/install-win | iex"])
        .kill_on_drop(true)
        .spawn()
...
mod tests {
    ...
    fn get_archive_entries(
        ...
        cyndra.load_project(&project_args).unwrap();
