...
use cyndra_common::models::deployment::get_deployments_table;
...
impl Cyndra {
    ...
    async fn init(&mut self, args: InitArgs, mut project_args: ProjectArgs) -> Result<()> {
        ...
        if project_args.name.is_none() {
            println!("How do you want to name your project? It will be hosted at ${{project_name}}.cyndraapp.rs.");
    ...
    async fn deploy(&self, client: &Client, args: DeployArgs) -> Result<CommandOutcome> {
        ...
        let mut last_state: Option<cyndra_common::deployment::State> = None;
        ...
        if let Some(ref new_deployment) = service.deployment {
            ...
            Ok(match new_deployment.state {
                cyndra_common::deployment::State::Crashed => CommandOutcome::DeploymentFailure,
                _ => CommandOutcome::Ok,
            })
        } else {
