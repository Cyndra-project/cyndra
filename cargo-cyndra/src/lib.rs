...
use cyndra_common::models::deployment::deployments_table_beta;
use cyndra_common::{
    constants::{
...
use cyndra_service::builder::{async_cargo_metadata, find_cyndra_packages};
use cyndra_service::{
...
impl Cyndra {
    ...
    pub async fn run(
        ...
        if matches!(
            ...
            let client =
                CyndraApiClient::new(self.ctx.api_url(), self.ctx.api_key().ok(), self.beta);
    ...
    async fn logs_beta(&self, args: LogsArgs) -> Result<CommandOutcome> {
        ...
        while let Some(Ok(msg)) = stream.next().await {
            if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                match serde_json::from_str::<cyndra_common::LogItemBeta>(&line) {
                    Ok(log) => {
                        if args.raw {
                            println!("{}", log.line);
                        } else {
