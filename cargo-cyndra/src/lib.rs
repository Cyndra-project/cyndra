            ...
            if let Err(err) = check_version(&service.executable_path) {
                ...
                if let Some(mismatch) = err.downcast_ref::<VersionMismatchError>() {
                    ...
                    return Err(err.context(
                        format!(
                            "Failed to verify the version of cyndra-runtime in {}. Is cargo targeting the correct binary?",
        ...
        for (i, service) in services.iter().enumerate() {
            ...
            signal_received = tokio::select! {
                res = Cyndra::spin_local_runtime(&run_args, service, &provisioner_server, i as u16, provisioner_port) => {
                    match res {
                        Ok(runtime) => {
                            Cyndra::add_runtime_info(runtime, &mut runtimes, &[&provisioner_server]).await?;
                        },
        ...
        client.delete_project(self.ctx.project_name(), true).await.map_err(|err| {
            if let Some(api_error) = err.downcast_ref::<ApiError>() {
                ...
                if api_error.message.contains("not ready") {
                    ...
                    println!();
                    println!("{}", "Try restarting the project with `cargo cyndra project restart` first.".yellow());
                    println!("{}", "This is needed to check for any resources linked to it.".yellow());
                    println!("{}", "For more help with deleting projects, visit https://docs.cyndra.rs/support/delete-project".yellow());
                    println!();
                    return err;
                }
            }
        ...
        println!(
            ...
            formatdoc!(
                ...
                WARNING:
                    ...
                    This will...
                    - Delete any cyndra-persist data in this project.
...
fn check_version(runtime_path: &Path) -> Result<()> {
    ...
    if !runtime_path.try_exists()? {
        bail!("cyndra-runtime is not installed");
    }
