...
use cyndra_builder::render_rust_dockerfile;
use cyndra_common::{
    constants::{
        headers::X_CARGO_cyndra_VERSION, other_env_api_url, EXAMPLES_REPO, RUNTIME_NAME,
        cyndra_API_URL, cyndra_CONSOLE_URL, STORAGE_DIRNAME, TEMPLATES_SCHEMA_VERSION,
    },
    models::{
...
use crate::builder::{async_cargo_metadata, build_workspace, find_cyndra_packages, BuiltService};
...
use crate::util::{
    ...
    get_templates_schema, is_dirty, open_gh_issue, read_ws_until_text, update_cargo_cyndra,
};
...
impl Cyndra {
    ...
    pub async fn run(mut self, args: CyndraArgs, provided_path_to_init: bool) -> Result<()> {
    ...
    async fn init(
        ...
        if needs_login {
            eprintln!("First, let's log in to your Cyndra account.");
        ...
        let template = match git_template {
            ...
            None => {
                ...
                let schema = if offline {
                    ...
                    get_templates_schema()
                        ...
                        .and_then(|s| {
                            ...
                            eprintln!(
                                "{}",
                                "Template list with incompatible version found. Consider upgrading Cyndra CLI. Falling back to internal list."
                                    .yellow()
                            );
                ...
                if let Some(schema) = schema {
                    ...
                    eprintln!("Cyndra works with many frameworks. Which one do you want to use?");
        ...
        let should_create_environment = if !interactive {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(format!(
                    r#"Create a project on Cyndra with the name "{name}"?"#
                ))
                .default(true)
                .interact()?;
            eprintln!();
        ...
        eprintln!("Run `cyndra run` to run the app locally.");
        if !should_create_environment {
            eprintln!("Run `cyndra deploy` to deploy it to Cyndra.");
        }
    ...
    async fn device_auth(&self, console_url: Option<String>) -> Result<String> {
        ...
        let url = token_message.url.unwrap_or_else(|| {
            format!(
                "{}/device-auth?token={}",
                console_url.as_deref().unwrap_or(cyndra_CONSOLE_URL),
                token
            )
        });
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        ...
        eprintln!(" -> Use `cyndra login` to log in again.");
    ...
    async fn logs(&self, args: LogsArgs) -> Result<()> {
        if args.follow {
            eprintln!("Streamed logs are not yet supported on the cyndra.dev platform.");
            return Ok(());
        }
        if args.tail.is_some() | args.head.is_some() {
            eprintln!("Fetching log ranges are not yet supported on the cyndra.dev platform.");
            return Ok(());
        }
        let client = self.client.as_ref().unwrap();
        let pid = self.ctx.project_id();
    ...
    async fn local_run(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let secrets = Cyndra::get_secrets(&run_args.secret_args, working_directory, true)?
            .unwrap_or_default();
        Cyndra::find_available_port(&mut run_args);
        ...
        let mut envs = vec![
            ...
            ("cyndra_HEALTHZ_PORT", healthz_port.to_string()),
    ...
    async fn local_docker_build(&self) -> Result<()> {
        ...
        fs::write(tempdir.join("cyndra_prebuild.sh"), "")?;
        ...
        // TODO?: Support custom cyndra.Dockerfile
        let dockerfile = tempdir.join("__cyndra.Dockerfile");
        ...
        let docker = tokio::process::Command::new("docker")
            ...
            .arg(dockerfile)
            .arg("--tag")
            .arg(format!("cyndra-build-{project_name}"))
            .arg(tempdir)
            .kill_on_drop(true)
            .spawn();
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        let secrets = Cyndra::get_secrets(&args.secret_args, working_directory, false)?;
...
mod tests {
    ...
    async fn get_archive_entries(
        ...
        let mut cyndra = Cyndra::new(crate::Binary::Cyndra, None).unwrap();
        cyndra
