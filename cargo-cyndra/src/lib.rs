...
{}"#,
    ...
    fn package_secret(&self, file: File) -> Result<Vec<u8>> {
        ...
        if secrets_path.exists() {
            archive_write
                .append_path_with_name(secrets_path, Path::new("cyndra").join("Secrets.toml"))?;
        }
...
mod tests {
    ...
    fn secrets_file_is_archived() {
        ...
        let file = cyndra.run_cargo_package(true).unwrap();
        ...
        let file = cyndra.run_cargo_package(true).unwrap();
        let new_file = cyndra.package_secret(file).unwrap();
