...
use cyndra_service::loader::{build_crate, Runtime};
use std::fmt::Write;
...
const BINARY_BYTES: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/release/cyndra-runtime"));
...
impl Cyndra {
    ...
    async fn auth(&mut self, auth_args: AuthArgs, client: &Client) -> Result<()> {
        ...
        println!("Run `cargo cyndra init --help` next");
    ...
    async fn local_run(&self, run_args: RunArgs) -> Result<()> {
        ...
        tokio::spawn(async move {
            while let Some(log) = stream.message().await.expect("to get log from stream") {
                let log: cyndra_common::LogItem = log.into();
                println!("{log}");
            }
        });
