...
use cyndra_common::constants::RUNTIME_NAME;
use tokio::io::AsyncBufReadExt;
use tracing::{debug, error, info, trace};

#[derive(Clone, Debug, Eq, PartialEq)]
/// This represents a compiled Cyndra service
...
fn extract_cyndra_toml_name(path: PathBuf) -> anyhow::Result<String> {
    let cyndra_toml =
        read_to_string(path.as_path()).map_err(|_| anyhow!("{} not found", path.display()))?;

    let toml: toml::Value =
        toml::from_str(&cyndra_toml).context("failed to parse Cyndra.toml")?;

    let name = toml
        .get("name")
        .context("couldn't find `name` key in Cyndra.toml")?
        .as_str()
        .context("`name` key in Cyndra.toml must be a string")?
        .to_string();
...
pub async fn build_workspace(
    ...
    if packages.is_empty() {
        bail!(
            "Did not find any packages that Cyndra can run. \
            Make sure your crate has a binary target that uses `#[cyndra_runtime::main]`."
        );
    }
...
pub fn find_cyndra_packages(metadata: &Metadata) -> anyhow::Result<Vec<Package>> {
    let mut packages = Vec::new();
    for member in metadata.workspace_packages() {
        // skip non-Cyndra-related crates
        ...
        let mut cyndra_deps = member
            ...
            .collect::<Vec<_>>();
        cyndra_deps.sort();
        info!(name = member.name, deps = ?cyndra_deps, "Found workspace member with cyndra dependencies");
...
async fn compile(
    ...
    for package in &packages {
        if package.features.contains_key("cyndra") {
            cmd.arg("--no-default-features").arg("--features=cyndra");
        }
    ...
    if !status.success() {
        bail!("Build failed. Is the Cyndra runtime missing?");
    }
