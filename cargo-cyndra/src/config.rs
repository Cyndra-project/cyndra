...
use cyndra_common::constants::API_URL_BETA;
use cyndra_common::{constants::API_URL_DEFAULT, ApiKey};
use tracing::trace;
...
/// Cyndra.toml schema (User-facing project-local config)
#[derive(Deserialize, Serialize, Default)]
pub struct ProjectConfig {
    // unused on new platform
    pub name: Option<String>,
    pub assets: Option<Vec<String>>,
    // unused in cargo-cyndra, used in new platform builder.
...
/// .cyndra/config.toml schema (internal project-local config)
#[derive(Deserialize, Serialize, Default)]
...
/// # Usage
/// ```rust,no_run
/// # use cargo_cyndra::config::{Config, GlobalConfig, GlobalConfigManager};
/// #
...
impl RequestContext {
    ...
    pub fn load_local_internal(&mut self, project_args: &ProjectArgs) -> Result<()> {
        ...
        trace!(
            "looking for .cyndra/config.toml in {}",
            workspace_path.display()
        );
        let local_manager =
            LocalConfigManager::new(workspace_path, ".cyndra/config.toml".to_string());
        let mut project_internal = Config::new(local_manager);
        if !project_internal.exists() {
            trace!("no local .cyndra/config.toml found");
            project_internal.replace(InternalProjectConfig::default());
        } else {
            trace!("found a local .cyndra/config.toml");
        ...
        // 2. Name from .cyndra/config.toml file
        match (&project_args.name_or_id, &config.id) {
            ...
            (None, Some(_)) => {
                trace!("using .cyndra/config.toml project id");
            }
            (None, None) => {
    ...
    pub fn get_local_config(
        ...
        trace!("looking for Cyndra.toml in {}", workspace_path.display());
        let local_manager = LocalConfigManager::new(workspace_path, "Cyndra.toml".to_string());
        let mut project = Config::new(local_manager);
        if !project.exists() {
            trace!("no local Cyndra.toml found");
            project.replace(ProjectConfig::default());
        } else {
            trace!("found a local Cyndra.toml");
            project.open()?;
        }
        ...
        match (&project_args.name_or_id, &config.name) {
            ...
            // If key exists in config then keep it as it is
            (None, Some(_)) => {
                trace!("using Cyndra.toml project name");
            }
    ...
    /// Get the API key from the `cyndra_API_KEY` env variable, or
    /// otherwise from the global configuration. Returns an error if
    /// an API key is not set.
    pub fn api_key(&self) -> Result<ApiKey> {
        let api_key = std::env::var("cyndra_API_KEY");

        if let Ok(key) = api_key {
            ApiKey::parse(&key).context("environment variable cyndra_API_KEY is invalid")
        } else {
...
mod tests {
    ...
    #[test]
    fn get_local_config_finds_name_in_cyndra_toml() {
