...
/// creates a `cargo-cyndra` Command instance with some reasonable defaults set.
fn cargo_cyndra_command() -> Command {
    let mut cmd = Command::cargo_bin("cargo-cyndra").unwrap();
    cmd.env(
        "cyndra_API",
...
fn default_prints_usage_to_stderr() {
    let mut cmd = cargo_cyndra_command();
    cmd.assert()
        .stderr(predicates::str::is_match("^cargo-cyndra.*\n\nUSAGE:").unwrap());
}
...
fn help_prints_usage_to_stdout() {
    ...
    cmd.arg("--help")
        .assert()
        .stdout(predicates::str::is_match("^cargo-cyndra.*\n\nUSAGE:").unwrap());
}
