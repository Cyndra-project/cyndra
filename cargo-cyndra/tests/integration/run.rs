...
/// Runs `cyndra run` in specified directory
async fn cyndra_run(working_directory: &str, external: bool) -> String {
...
async fn axum_hello_world() {
    let url = cyndra_run("../examples/axum/hello-world", false).await;
...
async fn rocket_secrets() {
    ...
    let url = cyndra_run("../examples/rocket/secrets", false).await;
...
mod needs_docker {
    ...
    async fn rocket_postgres() {
        let url = cyndra_run("../examples/rocket/postgres", false).await;
        let client = reqwest::Client::new();
