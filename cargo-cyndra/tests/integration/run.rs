use cargo_cyndra::{Args, Command, ProjectArgs, RunArgs, Cyndra};
...
async fn cargo_cyndra_run(working_directory: &str, external: bool) -> String {
    ...
    let runner = Cyndra::new().unwrap().run(Args {
...
async fn rocket_hello_world() {
    let url = cargo_cyndra_run("../examples/rocket/hello-world", false).await;
...
async fn rocket_secrets() {
    let url = cargo_cyndra_run("../examples/rocket/secrets", false).await;
...
async fn rocket_postgres() {
    let url = cargo_cyndra_run("../examples/rocket/postgres", false).await;
    let client = reqwest::Client::new();

    let post_text = client
        .post(format!("{url}/todo"))
        .body("{\"note\": \"Deploy to cyndra\"}")
        .send()
        .await
        .unwrap()
        .text()
    ...
    assert_eq!(post_text, "{\"id\":1,\"note\":\"Deploy to cyndra\"}");
    ...
    assert_eq!(request_text, "{\"id\":1,\"note\":\"Deploy to cyndra\"}");
}

#[tokio::test(flavor = "multi_thread")]
async fn axum_static_files() {
    let url = cargo_cyndra_run("../examples/axum/static-files", false).await;
    let client = reqwest::Client::new();
    ...
    assert!(
        request_text.contains("This is an example of serving static files with axum and cyndra.")
    );
}

#[tokio::test(flavor = "multi_thread")]
async fn cyndra_next() {
    let url = cargo_cyndra_run("../examples/next/hello-world", false).await;
    let client = reqwest::Client::new();
...
async fn rocket_authentication() {
    let url = cargo_cyndra_run("../examples/rocket/authentication", false).await;
    let client = reqwest::Client::new();
...
async fn actix_web_hello_world() {
    let url = cargo_cyndra_run("../examples/actix-web/hello-world", false).await;
...
async fn axum_hello_world() {
    let url = cargo_cyndra_run("../examples/axum/hello-world", false).await;
...
async fn tide_hello_world() {
    let url = cargo_cyndra_run("../examples/tide/hello-world", false).await;
...
async fn tower_hello_world() {
    let url = cargo_cyndra_run("../examples/tower/hello-world", false).await;
...
async fn warp_hello_world() {
    let url = cargo_cyndra_run("../examples/warp/hello-world", false).await;
...
async fn poem_hello_world() {
    let url = cargo_cyndra_run("../examples/poem/hello-world", false).await;
...
async fn poem_postgres() {
    let url = cargo_cyndra_run("../examples/poem/postgres", false).await;
    let client = reqwest::Client::new();
...
async fn poem_mongodb() {
    let url = cargo_cyndra_run("../examples/poem/mongodb", false).await;
    let client = reqwest::Client::new();
    ...
    assert_eq!(request_text, "{\"note\":\"Deploy to cyndra\"}");
}

#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn salvo_hello_world() {
    let url = cargo_cyndra_run("../examples/salvo/hello-world", false).await;
...
async fn thruster_hello_world() {
    let url = cargo_cyndra_run("../examples/thruster/hello-world", false).await;
...
async fn rocket_hello_world_with_router_ip() {
    let url = cargo_cyndra_run("../examples/rocket/hello-world", true).await;
