...
use cargo_cyndra::{Cyndra, CyndraArgs};
use clap::Parser;
use indoc::indoc;
...
async fn non_interactive_basic_init() {
    ...
    let args = CyndraArgs::parse_from([
        "cargo-cyndra",
        "init",
        "--api-key",
        "dh9z58jttoes3qvt",
...
async fn non_interactive_init_with_from_url() {
    ...
    let args = CyndraArgs::parse_from([
        ...
        "my-project",
        "--from",
        "https://github.com/cyndra-hq/cyndra-examples",
        "--subfolder",
        "tower/hello-world",
        temp_dir_path.to_str().unwrap(),
    ]);
    ...
    let cargo_toml = read_to_string(temp_dir_path.join("Cargo.toml")).unwrap();
    assert!(cargo_toml.contains("name = \"my-project\""));
    assert!(cargo_toml.contains("cyndra-tower ="));
...
async fn non_interactive_init_with_from_local_path() {
    ...
    let args = CyndraArgs::parse_from([
        ...
        "my-project",
        "--from",
        "../examples", // cargo runs the test from the cargo-cyndra folder.
        "--subfolder",
        "tower/hello-world",
        temp_dir_path.to_str().unwrap(),
    ]);
...
async fn non_interactive_init_from_local_path_with_workspace() {
    ...
    let cyndra_toml = read_to_string(temp_dir_path.join("Cyndra.toml")).unwrap();
    assert!(cyndra_toml.contains("name = \"my-project\""));
...
fn interactive_rocket_init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bin_path = assert_cmd::cargo::cargo_bin("cargo-cyndra");
    ...
    session.exp_string("container on Cyndra?")?;
    session.send("n")?;
    session.flush()?;
...
fn assert_valid_rocket_project(path: &Path, name: &str) {
    ...
    assert!(cargo_toml.contains("cyndra-runtime = "));
    assert!(cargo_toml.contains("cyndra-rocket = "));
