...
use cyndra_runtime::{CustomError, Error, tokio};
use std::{convert::Infallible, fmt, net::SocketAddr};

/// A wrapper type for [`Service`] so we can implement [`cyndra_runtime::Service`] for it.
...
impl<S, State> cyndra_runtime::Service for RamaService<Transport<S>, State>
where
...
#[cyndra_runtime::async_trait]
impl<S, State, Response> cyndra_runtime::Service for RamaService<Application<S>, State>
where
    ...
    State: Clone + Send + Sync + 'static,
{
    ...
    /// and binds to an address passed in by cyndra.
    async fn bind(self, addr: SocketAddr) -> Result<(), Error> {
        // cyndra only supports h1 between load balancer <=> web service,
        // h2 is terminated by cyndra's load balancer
        HttpServer::http1()
...
#[doc = include_str!("../README.md")]
pub type CyndraRamaTransport<S, State = ()> = Result<RamaService<Transport<S>, State>, Error>;

#[doc = include_str!("../README.md")]
pub type CyndraRamaApplication<S, State = ()> = Result<RamaService<Application<S>, State>, Error>;

pub use cyndra_runtime::{Error as CyndraError, Service as CyndraService};
