...
Before our app can use the database we have to add sqlx with some features: `cargo add sqlx -F postgres runtime-tokio-native-tls`. We will also enable the Postgres feature for cyndra with `cargo add cyndra-service -F sqlx-postgres`. 

Now back in the code we add a parameter with `#[shared::Postgres] pool: Database`. The `#[shared::Postgres]` annotation tells cyndra to provision a Postgres database using the [infrastructure from code design](https://www.cyndra.rs/blog/2022/05/09/ifc)!
...
`cargo cyndra project new` and `cargo cyndra deploy`
