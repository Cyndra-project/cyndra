...
Most server frameworks have a system called a 'router' which routes requests basde on various parameters - usually the URL path. In HTTP routing is typically a combination of the path and the request method (GET, POST, PUT etc.). The benefit of a router is that it allows splitting per path logic up, which makes building large systems with lots of endpoints easier to manage. 
...
The benefit of middleware *just* being functions is that can be separated out do different modules or even crates. Many 3rd party services may choose to expose there service as a middleware rather than a system, of complicated functions and having to deal with users passing the correct state into them.
...
Rockets fairings go beyond request and responses and can act as hooks into application startup and closing:
...
I was actually surprised when this worked, Axums handler parameter system is quite magic.
