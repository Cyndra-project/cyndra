...
mod _provisioner_client {
    ...
    pub async fn get_client(provisioner_uri: Uri) -> Client {
        ...
        let provisioner_service = tower::ServiceBuilder::new()
            .layer(cyndra_common::claims::ClaimLayer)
            .layer(cyndra_common::claims::InjectPropagationLayer)
            .service(channel);
...
mod _resource_recorder_client {
    ...
    use cyndra_common::backends::client::{self, ResourceDal};
    ...
    impl ResourceDal for Client {
        async fn get_project_resources(
            ...
            project_id: &str,
            token: &str,
        ) -> Result<Vec<cyndra_common::resource::Response>, client::Error> {
    ...
    mod tests {
        use serde_json::json;
        use cyndra_common::{database, resource};
