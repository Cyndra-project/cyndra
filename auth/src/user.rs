...
use cyndra_common::{backends::headers::XCyndraAdminSecret, claims::AccountTier, ApiKey, Secret};
...
impl UserManagement for UserManager {
    ...
    async fn upgrade_to_pro(
            ...
            .is_some()
        {
            ...
            .bind(name)
            .bind(CyndraSubscriptionType::Pro.to_string())
            .execute(&mut *transaction)
            .await?;
...
pub struct Subscription {
    ...
    pub r#type: CyndraSubscriptionType,
    pub quantity: i32,
...
pub enum CyndraSubscriptionType {
    Pro,
    Rds,
}
...
impl User {
    ...
    pub fn pro_subscription_id(&self) -> Option<&stripe::SubscriptionId> {
        self.subscriptions
            .iter()
            .find(|sub| matches!(sub.r#type, CyndraSubscriptionType::Pro))
            .map(|sub| &sub.id)
    }
    ...
    fn from_row(row: &PgRow) -> Result<Self, sqlx::Error> {
        Ok(Subscription {
            id: row
                ...
                .unwrap(),
            r#type: CyndraSubscriptionType::from_str(row.try_get("type").unwrap()).map_err(
                |err| sqlx::Error::ColumnDecode {
...
impl From<User> for cyndra_common::models::user::Response {
    ...
    UserManagerState: FromRef<S>,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        match parts.headers.typed_try_get::<XCyndraAdminSecret>() {
            Ok(Some(secret)) => {
