...
pub(crate) async fn app() -> TestApp {
    let pg_pool = pgpool_init(PG.get_unique_uri().as_str(), true)
        .await
        .unwrap();
    ...
    query("INSERT INTO users VALUES ($1, $2, $3, $4)")
        ...
        .bind(cyndra_auth::User::new_user_id())
        .execute(&pg_pool)
        .await
        .unwrap();
...
impl TestApp {
    ...
    pub async fn get_jwt_from_api_key(
        ...
        if let Some(key) = admin_api_key {
            request_builder = request_builder.header(X_cyndra_ADMIN_SECRET.to_string(), key)
        }
