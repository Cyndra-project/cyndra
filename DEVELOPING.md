...
```bash
docker compose -f docker-compose.rendered.yml -p cyndra-dev exec auth /usr/local/bin/cyndra-auth --db-connection-uri=postgres://postgres:postgres@control-db init-deployer --name gateway --key gateway4deployes
```
