...
> 🚨 NOTE 🚨: Big rewrites of Cyndra's infra and backends are ongoing.
...
- `proto` contains the gRPC server and client definitions to allow `deployer` to communicate with `provisioner`, and to allow the `deployer` and `cargo-cyndra` cli to communicate with the `alpha` runtime.
- `resources` contains various implementations of `ResourceBuilder`, which are consumed in the `codegen` to provision resources.
- `services` contains implementations of `Service` for common Rust web frameworks. Anything implementing `Service` can be deployed on Cyndra.
...
```bash
docker compose -f docker-compose.rendered.yml -p cyndra-dev exec auth /usr/local/bin/cyndra-auth --db-connection-uri=postgres://postgres:postgres@control-db init-deployer --user-id gateway --key gateway4deployes
```
...
```bash
cargo run -p cargo-cyndra -- --wd examples/rocket/hello-world project start
cargo run -p cargo-cyndra -- --wd examples/rocket/hello-world deploy
```
...
curl -H "Host: hello-world-rocket-app.unstable.cyndraapp.rs" localhost:8000
...
cargo run -p cargo-cyndra -- --wd examples/rocket/hello-world logs
```
...
Make sure an admin user is inserted into auth and that the key is used by cargo-cyndra. See above.

We're now ready to start a local run of the deployer:

```bash
OTLP_ADDRESS=http://127.0.0.1:4317 cargo run -p cyndra-deployer -- --provisioner-address http://localhost:3000 --auth-uri http://localhost:8008 --resource-recorder http://localhost:8007 --logger-uri http://localhost:8010 --proxy-fqdn local.rs --admin-secret dh9z58jttoes3qvt --local --project-id "01H7WHDK23XYGSESCBG6XWJ1V0" --project <name>
```

The `<name>` needs to match the name of the project that will be deployed to this deployer.
This is the `Cargo.toml` or `Cyndra.toml` name for the project.

Now that your local deployer is running, you can run commands against it using the cargo-cyndra CLI.
It needs to have the same project name as the one you submitted when starting the deployer above.

```bash
cargo run -p cargo-cyndra -- --wd <path> --name <name> deploy
```
...
should be set to test upgrading a user to Pro tier, or to use a Pro tier feature with cargo-cyndra CLI. On a local environment, that requires
