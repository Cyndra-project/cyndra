...
- `cargo-cyndra` is the CLI used by users to initialize, run, deploy and manage their projects and services on Cyndra.

### Libraries

- `api-client` is a reqwest client for calling the Cyndra API.
...
- `services` contains implementations of `Service` for common Rust frameworks to run on Cyndra. Anything implementing `Service` can be deployed on Cyndra.
...
```bash
cargo run --bin cyndra -- --wd examples/rocket/hello-world run
```
