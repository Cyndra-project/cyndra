...
use hyper::Uri;
use cyndra_common::{claims::Claim, resource::Type};
use cyndra_proto::{
...
impl ResourceManager for Persistence {
    ...
    async fn get_resource(
        &mut self,
        service_id: &Ulid,
        r#type: cyndra_common::resource::Type,
        claim: Claim,
    ...
    async fn delete_resource(
        ...
        project_name: String,
        service_id: &Ulid,
        resource_type: cyndra_common::resource::Type,
        claim: Claim,
    ) -> Result<ResultResponse> {
        if let Type::Database(db_type) = resource_type {
            let proto_db_type: cyndra_proto::provisioner::database_request::DbType =
                db_type.into();
...
impl AddressGetter for Persistence {
    #[instrument(skip_all, fields(cyndra.service.name = service_name, cyndra.project.name = service_name))]
