...
pub trait SecretRecorder: Clone + Send + Sync + 'static {
    ...
    async fn insert_secret(&self, name: &str, key: &str, value: &str) -> Result<(), Self::Err>;
}
...
pub trait SecretGetter: Clone + Send + Sync + 'static {
    ...
    async fn get_secrets(&self, name: &str) -> Result<Vec<Secret>, Self::Err>;
}
...
impl From<Secret> for cyndra_common::secret::Response {
