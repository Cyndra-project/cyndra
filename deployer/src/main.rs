...
use cyndra_deployer::{start, start_proxy, Args, DeployLayer, Persistence};
use cyndra_proto::runtime::{self, SubscribeLogsRequest};
use tokio::select;
...
// The `multi_thread` is needed to prevent a deadlock in cyndra_service::loader::build_crate() which spawns two threads
