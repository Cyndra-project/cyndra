...
use cyndra_proto::runtime::{self, runtime_client::RuntimeClient, StopRequest};
...
impl RuntimeManager {
    ...
    pub async fn get_runtime_client(
        ...
        let get_runtime_executable = || {
            if let Some(alpha_runtime) = alpha_runtime_path {
                ...
                if cfg!(debug_assertions) {
                    debug!("Installing cyndra-next runtime in debug mode from local source");
                    // If we're running deployer natively, install cyndra-runtime using the
                    ...
                    if let Ok(path) = path {
                        std::process::Command::new("cargo")
                            ...
                            .arg("next")
                            .output()
                            .expect("failed to install the local version of cyndra-runtime");
                    }
                }

                debug!("Returning path to cyndra-next runtime",);
        ...
        .await
        .context("failed to start cyndra runtime")?;
