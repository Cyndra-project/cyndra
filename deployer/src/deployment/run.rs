...
fn get_cached_output<T: DeserializeOwned>(
    cyndra_resource_type: &Type,
    config: &serde_json::Value,
    prev_resources: &[resource::Response],
) -> Option<T> {
    prev_resources
        .iter()
        .find(|resource| resource.r#type == *cyndra_resource_type && resource.config == *config)
        .and_then(|resource| {
            let cached_output = resource.data.clone();
            log(cyndra_resource_type, "Found cached output");
            match serde_json::from_value::<T>(cached_output) {
                Ok(output) => Some(output),
                Err(_) => {
                    log(cyndra_resource_type, "Failed to validate cached output");
                    None
                }
            }
        })
            ...
            }).collect::<anyhow::Result<Vec<_>>>()?.into_iter()
    {
        ...
        match cyndra_resource.r#type {
            resource::Type::Database(db_type) => {
                ...
                // be cached.
                let output = get_cached_output(&cyndra_resource.r#type, &serde_json::Value::Null, prev_resources.as_slice());
