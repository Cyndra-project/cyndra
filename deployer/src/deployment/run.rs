...
use cyndra_common::{backends::auth::Claim, storage_manager::ArtifactsStorageManager};
...
mod tests {
    ...
    async fn panic_in_main() {
        ...
        let handle_cleanup = |response: SubscribeStopResponse| match (
            ...
            (StopReason::Crash, mes) if mes.contains("Panic occurred in cyndra_service::main") => {
                cleanup_send.send(()).unwrap()
            }
            (_, mes) => panic!("expected stop due to crash: {mes}"),
        };
