...
async fn load(
    ...
    let cached_resources_contains_rds = resources.iter().any(|resource| {
        matches!(
            resource.r#type,
            cyndra_common::resource::Type::Database(cyndra_common::database::Type::AwsRds(_))
        )
    });
    ...
    debug!(cyndra.service.name = %service_name, "loading service");
    let response = runtime_client.load(load_request).await;

    debug!(cyndra.service.name = %service_name, "service loaded");
    match response {
        Ok(response) => {
            ...
            let loaded_resources_contains_rds = resources.iter().any(|resource| {
                matches!(
                    resource.r#type,
                    cyndra_common::resource::Type::Database(
                        cyndra_common::database::Type::AwsRds(_)
                    )
                )
            });
            ...
            if !cached_resources_contains_rds && loaded_resources_contains_rds {
                deployment_updater
                    .set_message(
                        &deployment_id,
                        "This deployment increased the cost of your subscription. To check your total, log in to the Cyndra web console.",
                    )
                    .await
                    .map_err(|err| {
