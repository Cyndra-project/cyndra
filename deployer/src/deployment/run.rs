...
use cyndra_common::project::ProjectName as ServiceName;
use cyndra_proto::runtime::{runtime_client::RuntimeClient, LoadRequest, StartRequest};
...
pub async fn task(
    ...
    let libs_path = artifacts_path.join("cyndra-libs");
    ...
    while let Some(built) = recv.recv().await {
        ...
        let cleanup = move |result: std::result::Result<
            std::result::Result<(), cyndra_service::Error>,
            JoinError,
        >| match result {
...
impl Built {
    ...
    async fn handle(
        ...
        cleanup: impl FnOnce(std::result::Result<std::result::Result<(), cyndra_service::Error>, JoinError>)
            + Send
            + 'static,
    ) -> Result<()> {
        // todo: refactor this?
...
async fn run(
    ...
    _cleanup: impl FnOnce(std::result::Result<std::result::Result<(), cyndra_service::Error>, JoinError>)
        + Send
        + 'static,
) {
    info!(
...
mod tests {
    ...
    const LIBS_PATH: &str = "/tmp/cyndra-libs-tests";
    ...
    async fn panic_in_bind() {
        ...
        >| {
            let result = result.unwrap();
            assert!(
                matches!(result, Err(cyndra_service::Error::BindPanic(ref msg)) if msg == "panic in bind"),
    ...
    async fn panic_in_main() {
        ...
        assert!(
            matches!(result, Err(Error::Run(cyndra_service::Error::BuildPanic(ref msg))) if msg == "main panic"),
    ...
    async fn missing_so() {
        ...
        assert!(
            matches!(
                result,
                Err(Error::Load(cyndra_service::loader::LoaderError::Load(_)))
            ),
