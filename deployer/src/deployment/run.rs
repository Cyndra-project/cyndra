...
use cyndra_service::{Environment, CyndraResourceOutput};
use tokio::{
    sync::Mutex,
...
impl Built {
    ...
    pub async fn handle(
        ...
        // For cyndra-next this is the path to the compiled .wasm file, which will be
        ...
        let alpha_runtime_path = if self.is_next {
            // The runtime client for next is the installed cyndra-next bin
            None
        } else {
        ...
        let prev_resources = resource_manager
            ...
            .inspect(|r| {
                if r.r#type == cyndra_common::resource::Type::Secrets {
...
async fn load(
    ...
    debug!(cyndra.project.name = %service_name, "loading service");
    ...
    debug!(cyndra.project.name = %service_name, "service loaded");
    match response {
        Ok(response) => {
...
fn get_cached_output<T: DeserializeOwned>(
    cyndra_resource: &ProvisionResourceRequest,
    prev_resources: &[resource::Response],
) -> Option<T> {
    prev_resources
        .iter()
        .find(|resource| {
            resource.r#type == cyndra_resource.r#type && resource.config == cyndra_resource.config
        })
        .and_then(|resource| {
            let cached_output = resource.data.clone();
            log(cyndra_resource.r#type, "Found cached output");
            match serde_json::from_value::<T>(cached_output) {
                Ok(output) => Some(output),
                Err(_) => {
                    log(cyndra_resource.r#type, "Failed to validate cached output");
                    None
                }
            }
        })
            ...
            }).collect::<anyhow::Result<Vec<_>>>()?.into_iter()
    {
        ...
        match cyndra_resource.r#type {
            resource::Type::Database(db_type) => {
                ...
                let _config: DbInput = serde_json::from_value(cyndra_resource.config.clone())
                    .context("deserializing resource config")?;

                let output = get_cached_output(&cyndra_resource, prev_resources.as_slice());
                let output = match output {
                    Some(o) => o,
                    None => {
                        log(cyndra_resource.r#type, "Provisioning...");
                        // ###
                ...
                resources_to_save.push(record_request::Resource {
                    r#type: cyndra_resource.r#type.to_string(),
