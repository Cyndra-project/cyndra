...
const BUILDS_PATH: &str = "/tmp/cyndra-builds";
...
/// when built.
const MARKER_FILE_NAME: &str = ".cyndra-marker";
...
mod tests {
    ...
    async fn extract_tar_gz_data() {
        let dir = TempDir::new("/tmp/cyndra-extraction-test").unwrap();
        let p = dir.path();
...
ff0e55bda1ff01000000000000000000e0079c01ff12a55500280000",
    ...
    async fn remove_old_build() {
        let dir = TempDir::new("/tmp/cyndra-remove-old-test").unwrap();
        let p = dir.path();
    ...
    async fn rename_build() {
        let dir = TempDir::new("/tmp/cyndra-rename-build-test").unwrap();
        let p = dir.path();
