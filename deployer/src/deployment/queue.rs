...
use cyndra_proto::builder::builder_client::BuilderClient;
use cyndra_proto::builder::BuildRequest;
                            ...
                            tokio::spawn(async move {
                                ...
                                match inner.build(req).await {
                                    Ok(inner) =>  {
                                        ...
                                        info!(id = %queued.id, "cyndra-builder finished building the deployment: image length is {} bytes, is_wasm flag is {} and there are {} secrets", response.image.len(), response.is_wasm, response.secrets.len());
                                    },
                                    Err(err) => error!(id = %queued.id, "cyndra-builder errored while building: {}", err)
                                };
                            });
                        }
    ...
    async fn handle(
        ...
        tokio::task::spawn_blocking(move || {
            while let Ok(message) = rx.recv() {
                ...
                let log = LogItem::new(
                    self.id,
                    cyndra_common::log::Backend::Deployer, // will change to Builder
                    match message {
        ...
        // Currently returns the first found cyndra service in a given workspace.
        let built_service = build_deployment(&project_path, tx.clone()).await?;

        // Get the Secrets.toml from the cyndra service in the workspace.
...
async fn extract_tar_gz_data(data: impl Read, dest: impl AsRef<Path>) -> Result<()> {
    ...
    for entry in archive.entries()? {
        ...
        // don't allow archive to overwrite cyndra internals
...
/// an embedded runtime for alpha, and a .wasm file for cyndra-next.
