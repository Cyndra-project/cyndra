...
mod tests {
    ...
    impl BuildQueueClient for StubBuildQueueClient {
        async fn get_slot(&self, _id: Uuid) -> Result<bool, cyndra_backends::client::Error> {
            Ok(true)
        }

        async fn release_slot(&self, _id: Uuid) -> Result<(), cyndra_backends::client::Error> {
            Ok(())
        }
    }
