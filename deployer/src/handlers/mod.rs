...
use cyndra_common::{log, secret, LogItem};
...
) -> Result<Json<Vec<cyndra_common::service::Response>>> {
...
async fn get_service(
...
) -> Result<Json<cyndra_common::service::Detailed>> {
    if let Some(service) = persistence.get_service_by_name(&service_name).await? {
        ...
        let response = cyndra_common::service::Detailed {
            name: service.name,
            deployments,
            resources,
            secrets,
...
) -> Result<Json<cyndra_common::service::Summary>> {
    if let Some(service) = persistence.get_service_by_name(&service_name).await? {
        ...
        let response = cyndra_common::service::Summary {
...
async fn post_service(
    ...
    stream: BodyStream,
) -> Result<Json<cyndra_common::deployment::Response>> {
...
async fn get_version() -> String {
    cyndra_service::VERSION.to_string()
}
