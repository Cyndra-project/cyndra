...
async fn get_service(
...
) -> Result<Json<cyndra_common::models::service::Detailed>> {
    if let Some(service) = persistence.get_service_by_name(&service_name).await? {
        ...
        let response = cyndra_common::models::service::Detailed {
            name: service.name,
            deployments,
            resources,
            secrets,
...
async fn get_build_logs(
    Extension(persistence): Extension<Persistence>,
    Path((_project_name, deployment_id)): Path<(String, Uuid)>,
) -> Result<Json<Vec<cyndra_common::log::BuildLogStream>>> {
    if let Some(deployment) = persistence.get_deployment(&deployment_id).await? {
        Ok(Json(
            persistence
...
async fn build_logs_websocket_handler(mut s: WebSocket, persistence: Persistence, id: Uuid) {
    ...
    for log in backlog.into_iter().filter_map(Log::into_build_log_stream) {
        match (log.state, log.message) {
            (cyndra_common::deployment::State::Building, Some(msg)) => {
            ...
            (cyndra_common::deployment::State::Building, None) => {}
            (cyndra_common::deployment::State::Queued, _)
            | (cyndra_common::deployment::State::Built, _)
            | (cyndra_common::deployment::State::Loading, _) => {}
            _ => {
