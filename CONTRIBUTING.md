...
Our goal with cyndra open-source maintenance is to foster a thriving, collaborative, and sustainable ecosystem around the project, which allows it to continue to grow and evolve over time.
...
curl --header "Host: {app}.unstable.cyndraapp.rs" localhost:8000/hello
```
...
```bash
AUTH_CONTAINER_ID=$(docker ps -aqf "name=cyndra-auth") \
...
`.config/cyndra/config.toml` as our `api_key`. By running the following curl command, we will request
...
Now copy the `token` value (just the value, not the key) from the curl response, and write it to your cyndra
config (which will be a file named `config.toml` in a directory named `cyndra` in one of 
...
echo "api_key = '<jwt>'" > ~/.config/cyndra/config.toml
```
...
>    | jq -r '.token' \
>    | read token; echo "api_key='$token'" > ~/.config/cyndra/config.toml
>```
...
```bash
cargo run -p cyndra-deployer -- --provisioner-address http://localhost:8000 --proxy-fqdn local.rs --admin-secret test-key --project <project_name>
```
