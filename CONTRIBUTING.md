...
> Note: The current [Makefile](https://github.com/cyndra-hq/cyndra/blob/main/Makefile) does not work on Windows systems by itself - if you want to build the local environment on Windows you could use [Windows Subsystem for Linux](https://learn.microsoft.com/en-us/windows/wsl/install). Additional Windows considerations are listed at the bottom of this page.
...
```bash
provisioner_address=$(docker inspect --format '{{(index .NetworkSettings.Networks "cyndra_default").IPAddress}}' cyndra_prod_hello-world-rocket-app_run)
cargo run -p cyndra-deployer -- --provisioner-address $provisioner_address --provisioner-port 8000 --proxy-fqdn local.rs --admin-secret test-key --project <project_name>
```
