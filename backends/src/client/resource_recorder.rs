...
use cyndra_proto::resource_recorder::Client;
use cyndra_proto::resource_recorder::ProjectResourcesRequest;
...
where
    T: ResourceDal,
{
    #[instrument(skip_all, fields(cyndra.project.id = project_id))]
    async fn get_project_resources(
        &mut self,
        project_id: &str,
        token: &str,
...
impl ResourceDal for Client {
    async fn get_project_resources(
        ...
        project_id: &str,
        token: &str,
    ) -> Result<Vec<cyndra_common::resource::Response>, Error> {
...
mod tests {
    ...
    use serde_json::json;
    use cyndra_common::{database, resource};
    use cyndra_proto::resource_recorder::{get_client, record_request, Client, RecordRequest};
