...
function update-examples-versions()
{
    ...
    rg "cyndra-service = \{ version" --files-with-matches service/ | xargs sed -i "s/cyndra-service = { version = \"[[:digit:]]*.[[:digit:]]*.[[:digit:]]*\"/cyndra-service = { version = \"$version\"/g"
    sed -i "s/cyndra-service = { version = \"[[:digit:]]*.[[:digit:]]*.[[:digit:]]*\"/cyndra-service = { version = \"$version\"/g" README.md
