    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let wrapper = quote! {
            async fn __cyndra_wrapper(
                ...
                #(let #fn_inputs = cyndra_service::#fn_inputs_builder::new()#fn_inputs_builder_options.build(#factory_ident, runtime).await?;)*
...
mod tests {
    ...
    fn from_with_inputs() {
        let mut input = parse_quote!(
            async fn complex(#[shared::Postgres] pool: PgPool) -> CyndraTide {}
        );
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn __cyndra_wrapper(
                ...
                let pool = cyndra_service::shared::Postgres::new().size("10Gb").public(false).build(factory, runtime).await?;
