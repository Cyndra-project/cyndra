    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn __cyndra_wrapper(
                ...
                let pool = cyndra_shared_db::Postgres::new().build(factory, runtime).await?;
                let redis = cyndra_shared_db::Redis::new().build(factory, runtime).await?;
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn __cyndra_wrapper(
                ...
                let pool = cyndra_shared_db::Postgres::new().size("10Gb").public(false).build(factory, runtime).await?;
