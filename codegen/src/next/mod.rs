...
impl ToTokens for Endpoint {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let route = quote!(.route(#route, cyndra_next::routing::#method(#function)));
...
pub(crate) fn wasi_bindings(app: App) -> proc_macro2::TokenStream {
    quote!(
        ...
        pub extern "C" fn __cyndra_Axum_call(
            ...
            let res = cyndra_next::block_on(__app(request));

            let (parts, mut body) = res.into_parts();

            // wrap and serialize response parts as rmp
            let response_parts = cyndra_next::ResponseWrapper::from(parts).into_rmp();
...
mod tests {
    ...
    fn endpoint_to_token() {
        ...
        let expected = quote!(.route("/hello", cyndra_next::routing::get(hello)));
    ...
    fn app_to_token() {
        ...
        let expected = quote!(
            async fn __app(
                request: cyndra_next::Request<cyndra_next::body::BoxBody>,
            ) -> cyndra_next::response::Response {
                use cyndra_next::Service;

                let mut router = cyndra_next::Router::new()
                    .route("/hello", cyndra_next::routing::get(hello))
                    .route("/goodbye", cyndra_next::routing::post(goodbye));
    ...
    fn parse_app() {
        let mut input = parse_quote! {
            ...
            #[cyndra_codegen::endpoint(method = post, route = "/goodbye")]
