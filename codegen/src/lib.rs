...
pub fn main(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[no_mangle]
        pub extern "C" fn _create_service() -> *mut dyn cyndra_service::Service {
            ...
            let obj = cyndra_service::IntoService::into_service((constructor));
            let boxed: Box<dyn cyndra_service::Service> = Box::new(obj);
            Box::into_raw(boxed)
        }
    };
...
struct Input {
    ...
    /// The cyndra_service path to the builder for this resource
    builder: Path,
}
    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let wrapper = quote! {
            async fn __cyndra_wrapper(
                ...
                #(let #fn_inputs = cyndra_service::#fn_inputs_builder::new().build(#factory_ident, runtime).await?;)*
    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn __cyndra_wrapper(
                ...
                let pool = cyndra_service::shared::Postgres::new().build(factory, runtime).await?;
                let redis = cyndra_service::shared::Redis::new().build(factory, runtime).await?;
