...
pub(crate) fn r#impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let tracing_setup = if cfg!(feature = "setup-tracing") {
        Some(quote! {
                use cyndra_runtime::colored::*;
                use cyndra_runtime::tracing_subscriber::{registry, fmt, prelude::*};
                cyndra_runtime::colored::control::set_override(true);
                ...
                println!(
                    ...
                    "cyndra-runtime = { version = \"*\", default-features = false }"
                        .white()
                        .italic(),
                    "=".repeat(52).yellow()
                );
    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn loader(
                ...
                use cyndra_runtime::Context;
                use cyndra_runtime::{Factory, ResourceBuilder};
                let pool = cyndra_runtime::get_resource(
                ...
                let redis = cyndra_runtime::get_resource(
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn loader(
                ...
                let pool = cyndra_runtime::get_resource (
                    cyndra_shared_db::Postgres::new().size(&cyndra_runtime::strfmt("10Gb", &vars)?).public(false),
                    &mut factory,
                    &mut resource_tracker,
                ).await.context(format!("failed to provision {}", stringify!(cyndra_shared_db::Postgres)))?;
                std::mem::drop(vars);
