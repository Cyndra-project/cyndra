    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn loader<S: cyndra_runtime::StorageManager>(
                ...
                let pool = cyndra_shared_db::Postgres::new().build(&mut factory).await.context(format!("failed to provision {}", stringify!(cyndra_shared_db::Postgres)))?;
                let redis = cyndra_shared_db::Redis::new().build(&mut factory).await.context(format!("failed to provision {}", stringify!(cyndra_shared_db::Redis)))?;
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn loader<S: cyndra_runtime::StorageManager>(
                ...
                let pool = cyndra_shared_db::Postgres::new().size("10Gb").public(false).build(&mut factory).await.context(format!("failed to provision {}", stringify!(cyndra_shared_db::Postgres)))?;
