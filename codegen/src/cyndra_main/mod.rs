    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn __cyndra_wrapper(
                ...
                let pool = cyndra_shared_db::Postgres::new().size("10Gb").public(false).build(factory, runtime).await.context(format!("failed to provision {}", stringify!(cyndra_shared_db::Postgres)))?;
