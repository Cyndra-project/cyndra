    ...
    quote! {
        fn main() {
            ...
            ::cyndra_runtime::tokio::runtime::Builder::new_multi_thread()
                ...
                .unwrap()
                .block_on(async {
                    ::cyndra_runtime::__internals::start(loader).await;
                })
        }
...
impl ToTokens for Loader {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let (factory_ident, resource_tracker_ident, extra_imports): (Ident, Ident, Option<Stmt>) =
            if self.fn_inputs.is_empty() {
                ...
                (
                    ...
                    Some(parse_quote!(
                        use ::cyndra_runtime::{Factory, IntoResource};
                    )),
                )
            };
        ...
        let loader = quote! {
            async fn loader(
                mut #factory_ident: ::cyndra_runtime::__internals::ProvisionerFactory,
                mut #resource_tracker_ident: ::cyndra_runtime::__internals::ResourceTracker,
            ) -> #return_type {
                ...
                #(
                    let #fn_inputs: #fn_inputs_types =
                        ::cyndra_runtime::__internals::get_resource::<_, _, #fn_inputs_types>(
...
mod tests {
    ...
    fn output_with_return() {
        ...
        let expected = quote! {
            async fn loader(
                mut _factory: ::cyndra_runtime::__internals::ProvisionerFactory,
                mut _resource_tracker: ::cyndra_runtime::__internals::ResourceTracker,
            ) -> CyndraSimple {
                use ::cyndra_runtime::__internals::Context;
                simple().await
            }
    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn loader(
                mut factory: ::cyndra_runtime::__internals::ProvisionerFactory,
                mut resource_tracker: ::cyndra_runtime::__internals::ResourceTracker,
            ) -> CyndraComplex {
                use ::cyndra_runtime::__internals::Context;
                use ::cyndra_runtime::{Factory, IntoResource};
                let pool: sqlx::PgPool = ::cyndra_runtime::__internals::get_resource::<_, _, sqlx::PgPool>(
                    cyndra_shared_db::Postgres::default(),
                    &mut factory,
                    &mut resource_tracker,
                )
                .await
                .context(format!("failed to provision {}", stringify!(cyndra_shared_db::Postgres)))?
                .into_resource()
                .await
                .context(format!("failed to initialize {}", stringify!(cyndra_shared_db::Postgres)))?;
                let redis: something::Redis = ::cyndra_runtime::__internals::get_resource::<_, _, something::Redis>(
                    cyndra_shared_db::Redis::default(),
                    &mut factory,
                    &mut resource_tracker,
                )
                .await
                .context(format!("failed to provision {}", stringify!(cyndra_shared_db::Redis)))?
                .into_resource()
                .await
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn loader(
                ...
                let pool: sqlx::PgPool = ::cyndra_runtime::__internals::get_resource::<_, _, sqlx::PgPool>(
                    cyndra_shared_db::Postgres::default().size(&::cyndra_runtime::__internals::strfmt("10Gb", &__vars)?).public(false),
                    &mut factory,
                    &mut resource_tracker,
                )
                .await
