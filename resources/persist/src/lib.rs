...
use cyndra_service::{DeploymentMetadata, ResourceFactory, ResourceInputBuilder};
use thiserror::Error;
...
impl ResourceInputBuilder for Persist {
    ...
    async fn build(self, factory: &ResourceFactory) -> Result<Self::Input, cyndra_service::Error> {
        ...
        PersistInstance::new(
            storage_path
                .join(PathBuf::from("cyndra-persist"))
                .join(PathBuf::from(project_name)), // separate persist directories per service
        )
        .map_err(|e| cyndra_service::Error::Custom(e.into()))
    }
}
