...
use cyndra_service::{
    error::{CustomError, Error as CyndraError},
...
impl From<Error> for cyndra_service::Error {
    fn from(error: Error) -> Self {
        let msg = match error {
            Error::UrlParseError(err) => format!("Cannot parse Turso Url: {}", err),
        };

        CyndraError::Custom(CustomError::msg(msg))
    }
}
...
impl Turso {
    async fn output_from_addr(
        &self,
        addr: &str,
    ) -> Result<<Turso as ResourceBuilder<Client>>::Output, cyndra_service::Error> {
        Ok(TursoOutput {
...
impl ResourceBuilder<Client> for Turso {
    ...
    async fn output(
        ...
        match factory.get_environment() {
            cyndra_service::Environment::Production => {
                if self.addr.is_empty() {
                    Err(CyndraError::Custom(CustomError::msg("missing addr")))
                } else {
    ...
    async fn build(config: &Self::Output) -> Result<Client, cyndra_service::Error> {
...
mod test {
    ...
    use async_trait::async_trait;
    use cyndra_service::{DatabaseReadyInfo, Environment, Factory, ResourceBuilder, ServiceName};
    ...
    impl MockFactory {
        fn new() -> Self {
            Self {
                temp_dir: Builder::new().prefix("cyndra-turso").tempdir().unwrap(),
                service_name: "cyndra-turso".to_string(),
    ...
    impl Factory for MockFactory {
        async fn get_db_connection(
            &mut self,
            _db_type: cyndra_service::database::Type,
        ...
        async fn get_secrets(
            &mut self,
        ) -> Result<std::collections::BTreeMap<String, String>, cyndra_service::Error> {
        ...
        fn get_build_path(&self) -> Result<std::path::PathBuf, cyndra_service::Error> {
            Ok(self.build_path())
        }

        fn get_storage_path(&self) -> Result<std::path::PathBuf, cyndra_service::Error> {
