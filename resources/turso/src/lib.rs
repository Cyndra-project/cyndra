...
impl Turso {
    async fn output_from_addr(
        ...
        addr: &str,
        environment: Environment,
    ) -> Result<TursoOutput, cyndra_service::Error> {
        Ok(TursoOutput {
            ...
            Environment::Deployment => {
                if self.addr.is_empty() {
                    ...
                    if !self.addr.starts_with("libsql://") && !self.addr.starts_with("https://") {
                        return Err(CyndraError::Custom(CustomError::msg(
...
impl IntoResource<Connection> for TursoOutput {
    async fn into_resource(self) -> Result<Connection, cyndra_service::Error> {
        let database = match self.environment {
            Environment::Deployment => Database::open_remote(
                self.conn_url.to_string(),
                self.token
                    .clone()
                    .ok_or(CyndraError::Custom(CustomError::msg(
        ...
        database
            .map_err(|err| CyndraError::Custom(err.into()))?
            .connect()
            .map_err(|err| CyndraError::Custom(err.into()))
    }
}
...
mod test {
    ...
    impl Factory for MockFactory {
        ...
        ) -> Result<cyndra_service::DatabaseInfo, cyndra_service::Error> {
