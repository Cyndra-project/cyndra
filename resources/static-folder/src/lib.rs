...
use cyndra_service::{
    error::{CustomError, Error as CyndraServiceError},
...
impl<'a> ResourceBuilder<PathBuf> for StaticFolder<'a> {
    ...
    async fn build(build_data: &Self::Output) -> Result<PathBuf, cyndra_service::Error> {
...
impl From<Error> for cyndra_service::Error {
    fn from(error: Error) -> Self {
        ...
        CyndraServiceError::Custom(CustomError::msg(msg))
    }
}
...
mod tests {
    ...
    use async_trait::async_trait;
    use cyndra_service::{DatabaseReadyInfo, Factory, ResourceBuilder};
