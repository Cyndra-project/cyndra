...
impl IntoResource<Operator> for OpendalOutput {
    async fn into_resource(self) -> Result<Operator, cyndra_service::Error> {
...
mod test {
    use super::*;
    use cyndra_service::{Environment, Secret};
    ...
    impl Factory for MockFactory {
        ...
        async fn get_secrets(
            &mut self,
        ) -> Result<BTreeMap<String, Secret<String>>, cyndra_service::Error> {
            let secrets = self
                .secrets
                .iter()
