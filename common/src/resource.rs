...
/// Return this struct as a resource config to make Cyndra provision it
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ProvisionResourceRequest {
    /// The version of the config+data schema for this Cyndra resource
    pub version: u32,
    ...
    fn from(value: ProvisionResourceRequest) -> Self {
        Self {
            r#type: match value.r#type {
                ...
                r => panic!("Resource not supported on cyndra.dev: {r}"),
            },
            config: value.config,
        }
    }
...
#[derive(Deserialize)]
#[serde(untagged)] // Try deserializing as a Cyndra resource, fall back to a custom value
pub enum ResourceInput {
    Cyndra(ProvisionResourceRequest),
    Custom(Value),
}
...
pub enum ResourceInputBeta {
    Cyndra(ProvisionResourceRequestBeta),
    Custom(Value),
}
...
/// Returned when provisioning a Cyndra resource
#[derive(Serialize, Deserialize)]
pub struct CyndraResourceOutput<T> {
    /// The output type for this Cyndra resource,
