...
/// The input given to Cyndra DB resources
...
/// The output produced by Cyndra DB resources
...
///
/// Use [`Self::connection_string_cyndra`] when running on Cyndra,
...
impl DatabaseInfo {
    ...
    /// For connecting to the db from inside the Cyndra network
    pub fn connection_string_cyndra(&self) -> String {
        format!(
            "{}://{}:{}@{}:{}/{}",
            self.engine,
            self.role_name,
            self.role_password.expose(),
            self.hostname_cyndra,
            self.port,
            self.database_name,
        )
    }
...
pub struct DatabaseInfoBeta {
    ...
    database_name: String,
    port: String,
    #[serde(alias = "hostname_cyndra")] // compatibility to parse from a `DatabaseInfo`
    hostname: String,
...
pub struct VersionInfo {
    ...
    /// Latest version of cargo-cyndra compatible with this gateway.
    pub cargo_cyndra: semver::Version,
    /// Latest version of cyndra-deployer compatible with this gateway.
    pub deployer: semver::Version,
    /// Latest version of cyndra-runtime compatible with the above deployer.
