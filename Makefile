SRC_CRATES=deployer common codegen cargo-cyndra proto provisioner service
...
STACK=cyndra-prod
APPS_FQDN=cyndraapp.rs
DB_FQDN=db.cyndra.rs
CONTAINER_REGISTRY=public.ecr.aws/cyndra
DD_ENV=production
...
.PHONY: images clean src up down deploy cyndra-% postgres docker-compose.rendered.yml test bump-% deploy-examples publish publish-% --validate-version

clean:
	rm .cyndra-*
	rm docker-compose.rendered.yml

images: cyndra-provisioner cyndra-deployer cyndra-gateway cyndra-auth cyndra-resource-recorder postgres panamax otel
...
up: $(DOCKER_COMPOSE_FILES)
	if [ "$(cyndra_DETACH)" = "disable" ]; then $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE) $(addprefix -f ,$(DOCKER_COMPOSE_FILES)) -p $(STACK) up; else $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE) $(addprefix -f ,$(DOCKER_COMPOSE_FILES)) -p $(STACK) up --detach; fi
...
# Bunch of targets to make bumping the cyndra version easier
#
...
# Deploy all our example using the command set in cyndra-command
# Usage: make deploy-example cyndra-command="cargo cyndra" -j 2
...
deploy-examples/%:
	cd examples/$(*); $(cyndra-command) project stop || echo -e "\x1B[33m>> Nothing to remove for $*\x1B[39m"
	sleep 5
	cd examples/$(*); $(cyndra-command) project start
	sleep 5
	cd examples/$(*); $(cyndra-command) deploy
...
publish: publish-resources publish-cargo-cyndra
...
publish-cargo-cyndra: publish-resources/secrets
	cd cargo-cyndra; cargo publish
