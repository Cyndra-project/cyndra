...
#[tokio::test]
#[should_panic(expected = "Build failed. Is the Cyndra runtime missing?")]
async fn not_cyndra() {
    let (tx, _) = tokio::sync::mpsc::channel::<String>(256);
    let project_path = format!("{}/tests/resources/not-cyndra", env!("CARGO_MANIFEST_DIR"));
...
#[tokio::test]
#[should_panic(
    expected = "Did not find any packages that Cyndra can run. Make sure your crate has a binary target that uses `#[cyndra_runtime::main]`."
)]
async fn not_bin() {
    let (tx, _) = tokio::sync::mpsc::channel::<String>(256);
