...
async fn not_cyndra() {
    ...
    assert!(
        so_path
            .display()
            .to_string()
            .ends_with("tests/resources/not-cyndra/target/debug/libnot_cyndra.so"),
...
#[tokio::test]
#[should_panic(
    expected = "Your Cyndra project must be a library. Please add `[lib]` to your Cargo.toml file."
)]
async fn not_lib() {
    let (tx, _) = crossbeam_channel::unbounded();
