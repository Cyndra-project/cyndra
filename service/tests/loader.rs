...
use cyndra_service::loader::{Loader, LoaderError};
use cyndra_service::{Error, Factory};
...
fn not_cyndra() {
    Command::new("cargo")
        .args(["build", "--release"])
        .current_dir("tests/resources/not-cyndra")
        .spawn()
        .unwrap()
        .wait()
        .unwrap();
    ...
    let result =
        Loader::from_so_file("tests/resources/not-cyndra/target/release/libnot_cyndra.so");
