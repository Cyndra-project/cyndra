...
use cyndra_common::constants::STORAGE_DIRNAME;
pub use cyndra_common::{
    database,
    deployment::{DeploymentMetadata, Environment},
    resource::{self, CyndraResourceOutput},
    secrets::Secret,
...
/// Allows implementing plugins for the Cyndra main function.
///
/// ## Creating your own Cyndra plugin
///
/// You can add your own implementation of this trait along with [`IntoResource`] to customize the
/// input type `R` that gets into the Cyndra main function on an existing resource.
///
/// You can also make your own plugin, for example to generalise the connection logic to a third-party service.
/// One example of this is `cyndra-qdrant`.
///
/// Please refer to `cyndra-examples/custom-resource` for examples of how to create a custom resource. For more advanced provisioning
/// of custom resources, please [get in touch](https://discord.gg/cyndra) and detail your use case. We'll be interested to see what you
...
pub trait ResourceInputBuilder: Default {
    ...
    ///
    /// If the input is a [`cyndra_common::resource::ProvisionResourceRequest`],
    /// then the resource will be provisioned and the [`ResourceInputBuilder::Output`]
    /// will be a [`cyndra_common::resource::CyndraResourceOutput<T>`] with the resource's associated output type.
    type Input: Serialize + DeserializeOwned;

    /// The output from provisioning this resource.
    ///
    /// For custom resources that don't provision anything from Cyndra,
...
/// base resource into the end type exposed to the Cyndra main function.
#[async_trait]
...
#[async_trait]
impl<R: Serialize + DeserializeOwned + Send> IntoResource<R> for CyndraResourceOutput<R> {
    async fn into_resource(self) -> Result<R, crate::Error> {
        Ok(self.output)
    }
}
...
pub trait Service: Send {
    ...
    ///
    /// The passed [`SocketAddr`] receives proxied HTTP traffic from your Cyndra subdomain (or custom domain).
