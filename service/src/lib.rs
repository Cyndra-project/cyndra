...
///
/// Also see the [cyndra_runtime::main] macro.
#[async_trait]
...
/// [`Factory`] can be used to provision resources on Cyndra's servers if your service will need any.
///
/// Please refer to `cyndra-examples/custom-resource` for examples of how to create custom resource. For more advanced provisioning
...
///
/// Use the [cyndra_runtime::main] macro to expose your implementation to the deployment backend.
#[async_trait]
pub trait Service: Send + Clone {
    ...
    ///
    /// Default implementation assumes the service is bound to `addr` and responds with 200 OK on '/_cyndra/healthz'.
    /// Override this if not relevant.
    async fn health_check(self, addr: &SocketAddr) -> Result<(), error::Error> {
        reqwest::get(reqwest::Url::parse(&format!("http://{addr}/_cyndra/healthz")).unwrap())
            .await
