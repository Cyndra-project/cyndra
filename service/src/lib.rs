...
//!
//! ## Usage
//! Start by installing the [`cargo cyndra`](https://docs.rs/crate/cargo-cyndra/latest) subcommand by runnning the following in a terminal:
//!
//! ```bash
...
//!
//! ```toml
//! cyndra-service = { version = "0.3.3", features = ["web-rocket", "sqlx-postgres"] }
//! ```
//!
...
/// The following types can be returned from a `#[cyndra_service::main]` function and enjoy first class service support in cyndra. Be sure to also enable the correct feature on
...
/// Cyndra is able to manage service dependencies for you. These are passed in as inputs to your `#[cyndra_service::main]` function:
/// ```rust,no_run
/// use sqlx::PgPool;
/// use cyndra_service::CyndraRocket;
///
