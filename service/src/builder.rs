...
/// This represents a compiled alpha or cyndra-next service.
...
impl BuiltService {
    ...
    pub fn service_name(&self) -> anyhow::Result<ProjectName> {
        let cyndra_toml_path = self.working_directory.join("Cyndra.toml");
...
fn extract_cyndra_toml_name(path: PathBuf) -> anyhow::Result<String> {
    let cyndra_toml = read_to_string(path).context("Cyndra.toml not found")?;
...
pub async fn build_workspace(
    ...
    if !manifest_path.exists() {
        return Err(anyhow!("failed to read the Cyndra project manifest"));
    }
...
pub async fn clean_crate(project_path: &Path, release_mode: bool) -> anyhow::Result<Vec<String>> {
    ...
    let manifest_path = project_path.join("Cargo.toml");
    if !manifest_path.exists() {
        bail!("failed to read the Cyndra project manifest");
    }
...
fn ensure_cdylib(package: &Package) -> anyhow::Result<()> {
    if package.targets.iter().any(is_cdylib) {
        ...
        bail!("Your Cyndra next project must be a library. Please add `[lib]` to your Cargo.toml file.")
    }
}
